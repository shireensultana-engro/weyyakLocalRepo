...
func (hs *HandlerService) UpdatePageOrder(c *gin.Context) {
	...
	// 	return
	// }
	// fmt.Println(response.PublishingPlatformOrderedDetails.TargetPlatform0[2].PageId, "/////////////")
...
func (hs *HandlerService) CreateOrUpdatePageDetails(c *gin.Context) {
	...
	if c.Param("id") == "" {
		var pagekey Page
		tx.Debug().Table("page").Select("max(page_key)+1 as page_key").Find(&pagekey)
		...
		pageres := Page{Id: pageRequest.PageId, EnglishTitle: pageRequest.EnglishTitle, ArabicTitle: pageRequest.ArabicTitle, PageOrderNumber: 1, EnglishPageFriendlyUrl: pageRequest.EnglishPageFriendlyUrl, ArabicPageFriendlyUrl: pageRequest.ArabicPageFriendlyUrl, EnglishMetaTitle: pageRequest.EnglishMetaTitle, ArabicMetaTitle: pageRequest.ArabicMetaTitle, EnglishMetaDescription: pageRequest.EnglishMetaDescription, ArabicMetaDescription: pageRequest.ArabicMetaDescription, IsDisabled: IsCheck, PageKey: pageRequest.PageKey, PageType: pagetype, CreatedAt: time.Now(), ModifiedAt: time.Now(), HasMenuPosterImage: menuposterimage, HasMobileMenuPosterImage: mobilemenuposterimage, HasMobileMenu: mobilemenu}
		...
		for _, req := range pageRequest.PublishingPlatforms {
			...
			newRecord.PageId = pageRequest.PageId
		...
		go PageFileUploadGcp(pageRequest.NonTextualData, pageres.Id)
		/* update dirty count in page_sync table */
		...
		/*Upload Images In S3 with Respect to PageId */
		go PageFileUploadGcp(pageRequest.NonTextualData, c.Param("id"))
		/* update dirty count in page_sync table */
...
func PageFileUploadGcp(request NonTextualData, pageId string) {
	...
	for _, item := range newarr {
		...
		if err := copyObject(ctx, client, bucketName, source, bucketName, destination); err != nil {
		...
		// Open a file for writing
		file, ferr := os.Create(filetrim[0])
		if ferr != nil {
			fmt.Println(ferr)
		}
		defer file.Close()
		...
		errorr := SizeUploadFileToGcp(ctx, client, filetrim[0], pageId)
		if errorr != nil {
			fmt.Println("error in uploading size upload", errorr)
		}
		if ferr == nil {
...
// func SizeUploadFileToGcp(ctx context.Context, client *storage.Client, fileName string, contentId string) error {
// 	bucketName := os.Getenv("BUCKET_NAME")
// 	file, err := os.Open(fileName)
// 	fmt.Println("ffilleee" , file)
// 	if err != nil {
// 		return err
// 	}
// 	defer file.Close()
...
// 		wc.ObjectAttrs.ACL = []storage.ACLRule{{Entity: storage.AllUsers, Role: storage.RoleReader}}
// 		wc.ObjectAttrs.ContentDisposition = "attachment"
// 		wc.ObjectAttrs.StorageClass = "STANDARD"
...
func calculateCRC32C(buffer []byte) uint32 {
	return crc32.Checksum(buffer, crc32.MakeTable(crc32.Castagnoli))
}
...
func SizeUploadFileToGcp(ctx context.Context, client *storage.Client, fileName string, contentId string) error {
	...
	sizeValue := [...]string{
		"222x394", "160x290", "60X60", "100x100/", "150x150/", "200x200/", "250x250/", "270x270/",
		"300x300/", "420x420/", "450x450/", "570x570/", "600x600/", "620x620/", "800x384/",
		"800x800/", "811x811/", "900x900/", "2048x670/", "1125x240/",
	}
	...
	for i := 0; i < len(sizeValue); i++ {
		...
		wc.ObjectAttrs.StorageClass = "STANDARD"
		wc.ObjectAttrs.CacheControl = "no-cache"
		wc.CRC32C = calculateCRC32C(buffer) // Calculate CRC32C for the content
		wc.SendCRC32C = true
...
func copyObject(ctx context.Context, client *storage.Client, srcBucket, srcObject, dstBucket, dstObject string) error {
