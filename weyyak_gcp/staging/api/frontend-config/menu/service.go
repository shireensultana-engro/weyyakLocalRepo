...
func (hs *HandlerService) TvHomePage(c *gin.Context) {
	...
	if RedisResponse.Value != "" {
		...
		url := "https://msapifo-stg-me.weyyak.z5.com/v1/" + language + "/menu?device=" + device
		method := "GET"
		client := &http.Client{}
		...
		for _, singlePage := range menuPage.Data {
			...
			url := "https://msapifo-stg-me.weyyak.z5.com/v1/" + language + "/menu/" + strconv.Itoa(singlePage.ID) + "?cascade=2&country=" + country
			method := "GET"
			fmt.Println(url)
			client := &http.Client{}
			req, err := http.NewRequest(method, url, nil)
