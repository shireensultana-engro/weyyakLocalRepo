...
func (hs *HandlerService) GetMenuDetails(c *gin.Context) {
	...
	GCPurl, err := common.GetCurlCall(os.Getenv("GCP_URLFORCONFIG"))
	if err != nil {
		c.JSON(http.StatusInternalServerError, err)
	}
	var result interface{}
	err = json.Unmarshal(GCPurl, &result)
	if err != nil {
		c.JSON(http.StatusInternalServerError, err)
	}
	var mapResult map[string]interface{}
	err1 := json.Unmarshal(GCPurl, &mapResult)
	if err1 != nil {
		c.JSON(http.StatusInternalServerError, err)
	}
			...
			if playlist.PlaylistType == "pagecontent" {
				...
				if err := fdb.Raw(`
							...
							where
									...
									AND (
										rights_start_date::timestamp < now()
										and
										rights_end_date::timestamp > now()
									)
						`, Ids, countryCodeInt, language, platformNameInt).Scan(&contentFragmentDetails).Error; err != nil {
					c.JSON(http.StatusInternalServerError, serverError)
					return
...
func (hs *HandlerService) GetTopMenuDetails(c *gin.Context) {
	...
	var mapResult map[string]interface{}
	err1 := json.Unmarshal(url, &mapResult)
	if err1 != nil {
		c.JSON(http.StatusInternalServerError, err)
	}
	// Check if "restrictedCountries" key exists and is of type []interface{}
