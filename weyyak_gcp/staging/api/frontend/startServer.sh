export SERVICE_PORT=3003 
export DB_SERVER=34.18.68.74
export DB_PORT=5432 
export DB_USER=appuser 
export DB_PASSWORD=WMFPfp6UGIQYIAY 
export DB_DATABASE=wk_frontend 
export CONTENT_DB_DATABASE=wyk_content 
export FRONTEND_CONFIG_DB_DATABASE=wyk_frontend_config 
export USER_DB_DATABASE=wk_user_management 
export DEFAULT_PAGE_SIZE=20 
export CONTENT_COMMON_API_URL= 
export CMS=https://msapifo-stg-me.weyyak.z5.com/v1/ 
export UM=https://msapifo-stg-me.weyyak.z5.com/v1/ 
export IMAGES=https://contents-stg.weyyak.z5.com/ 
export GEO_LOCATION=https://geo-stg.weyyak.com 
export VIDEO_API=https://api-weyyak.akamaized.net/get_info/ 
export AD_TAG_URL=https://s3.ap-south-1.amazonaws.com/z5xml/mobile_apps_ads_ios.xml 
export BASE_URL=https://msapifo-stg-me.weyyak.z5.com/ 
export USER_ACTIVITY_URL=https://msapistage-me-events.z5.com/event/activity 
export USER_LOG_URL=https://msapistage-me-events.z5.com/event/log 
export CONTENT_TYPE_URL=https://msapifo-stg-me.weyyak.z5.com/v1/en/contents/contentType?contentType= 
export CONTENT_TYPE_URL_PAGINATION="&pageNo=1&OrderBy=desc&RowCountPerPage=50&IsPaging=0" 
export S3_BUCKET=z5content 
export S3_REGION=ap-south-1 
export S3_ID=AKIAYOGUWMUMLWREWLMX 
export S3_SECRET=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 
export S3_URLFORCONFIG=https://z5content.s3.ap-south-1.amazonaws.com/configprod.json 
export REDIS_CACHE_URL=https://msapistage-me-events.z5.com/v1/cache 
export REDIS_CONTENT_KEY=GOAPISTAGE 
export DOTNET_URL=https://api-backoffice-production.weyyak.com/oauth2/tokendata?access_token= 
export GCP_URLFORCONFIG=https://storage.googleapis.com/weyyak-stage/configstagemigration.json 
export BUCKET_NAME=weyyak-stage 
export TYPE=service_account 
export PROJECT_ID=wyk-stage 
export PRIVATE_KEY_ID=fd9feef48aacb563b85d9a1e4b957a325c1e9187 
export PRIVATE_KEY="-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n" 
export CLIENT_EMAIL=weyyak-stage@wyk-stage.iam.gserviceaccount.com 
export CLIENT_ID=102280627070321861141 
export AUTH_URI=https://accounts.google.com/o/oauth2/auth 
export TOKEN_URI=https://oauth2.googleapis.com/token 
export AUTH_PROVIDER_X509_CERT_URL=https://www.googleapis.com/oauth2/v1/certs 
export CLIENT_X509_CERT_URL=https://www.googleapis.com/robot/v1/metadata/x509/weyyak-stage%40wyk-stage.iam.gserviceaccount.com 
export UNIVERSE_DOMAIN=googleapis.com

go run *.go