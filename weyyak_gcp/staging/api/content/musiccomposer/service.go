...
func (hs *HandlerService) GetAllMusicComposer(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"data": MusicComposer})
}
...
func (hs *HandlerService) CreateMusicComposer(c *gin.Context) {
	...
	if err := db.Debug().Create(&musicComposer).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"message": err, "Status": http.StatusBadRequest})
		return
	} else {

		c.JSON(http.StatusOK, gin.H{"message": "MusicComposer Created Successfully.", "Status": http.StatusOK})
		return
	}
}
