...
func (hs *HandlerService) GetEpisodeBasedOnSeasonId(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"pagination": pages, "data": finalResult})
}
...
func (hs *HandlerService) CreateOrUpdateEpisodes(c *gin.Context) {
	...
	if c.Param("id") == "" {
		...
		if contentkeyresult := tx.Debug().Table("episode").Select("max(episode_key) as episode_key").Find(&episodeKey).Error; contentkeyresult != nil {
			tx.Rollback()
		...
		if seasonresult := tx.Debug().Table("season").Select("content_id").Where("id=?", episodes.SeasonId).Find(&seasonDetails).Error; seasonresult != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"message": seasonresult.Error(), "status": http.StatusInternalServerError})
		}
		...
		c.JSON(http.StatusOK, gin.H{"data": finEpiId})
		...
		if checkEpisode < 1 {
			c.JSON(http.StatusBadRequest, gin.H{"message": "error_season_not_found", "status": http.StatusBadRequest})
			return
		}
...
func (hs *HandlerService) GetSeasonDetailsBySeasonId(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"data": seasonResult})
	return
}
	...
	for k := 0; k < len(newarr); k++ {
		...
		if e == nil {
			...
			if err == nil {
				...
				err = SizeUploadFileToGcp(client, filetrim[0], episodeId, contentId, seasonId)
				if err != nil {
...
func SizeUploadFileToGcp(client *storage.Client, fileName string, episodeId string, contentId string, seasonId string) error {
	file, err := os.Open(fileName)
	if err != nil {
		return err
	}
