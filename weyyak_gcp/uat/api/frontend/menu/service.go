...
func (hs *HandlerService) CreatePageResponse(c *gin.Context) {
	...
	l.JSON(c, http.StatusOK, gin.H{"Message": "Success", "Status": http.StatusOK})
	return
}
...
func (hs *HandlerService) GetMenuDetails(c *gin.Context) {
	...
	if RedisResponse.Value != "" {
		...
		var countryCodeInt string
		if c.Request.URL.Query()["country"] != nil {
			countryCode = strings.ToUpper(c.Request.URL.Query()["country"][0])
			var i int32 = common.Countrys(countryCode)
			countryCodeInt = strconv.Itoa(int(i))
		...
		var platformName string = "web"
		var platformNameInt string = "0"

		if c.Request.URL.Query()["platform"] != nil {
			platformName = c.Request.URL.Query()["platform"][0]
			var j int32 = common.DeviceIds(platformName)
			platformNameInt = strconv.Itoa(int(j))
		}
		...
		if details.HasMobileMenu == true {
			imageryDetails.MobileMenu = os.Getenv("IMAGES") + strings.ToLower(details.Id) + "/mobile-menu"
			imageryDetails.MobilePosterImage = os.Getenv("IMAGES") + strings.ToLower(details.Id) + "/menu-poster-image"
			imageryDetails.MobileMenuPosterImage = os.Getenv("IMAGES") + strings.ToLower(details.Id) + "/mobile-menu-poster-image"
		} else {
		...
		if slider.SliderKey != 0 {
			...
			if slider.BlackAreaPlaylistId != "" || slider.RedAreaPlaylistId != "" || slider.GreenAreaPlaylistId != "" {
				...
				for _, playlist := range playlists {
					...
					// if err := fdb.Table("content_fragment").Select("details::text as details").Where("content_id in(?) and country=? and language=? and platform=?", Ids, countryCodeInt, language, platformName).Find(&contentFragmentDetails).Error; err != nil {
					// 	l.JSON(c, http.StatusInternalServerError, serverError)
					// 	return
					// }
					...
					if err := fdb.Raw(`
							...
							where
									content_id in (?)
									and ? = ANY(string_to_array(replace(replace(country, '[', ''), ']', ''), ', '))
									and language = ?
									and ? = ANY(string_to_array(replace(replace(platform, '[', ''), ']', ''), ', '))
									and (
										details->>'scheduling_date_time' IS NULL 
										OR (
											to_timestamp(details->>'scheduling_date_time', 'YYYY-MM-DD HH24:MI:SS') < now() 
			...
			featuredDetails.MoreButton = slider.MoreButton
			featuredDetails.PlayWatchTrailer = slider.PlayWatchTrailer
	...
	l.JSON(c, http.StatusOK, gin.H{"data": menuPage})
	return
}
...
// @Success 200 {array} SideMenuDetails
...
func (hs *HandlerService) GetSideMenuPageDetails(c *gin.Context) {
	...
	if c.Request.URL.Query()["device"] == nil || c.Request.URL.Query()["device"][0] == "" {
		l.JSON(c, http.StatusNotFound, gin.H{"error": "Not Found"})
		return
	}
	if c.Request.URL.Query()["country"] != nil {
		CountryCode = strings.ToUpper(c.Request.URL.Query()["country"][0])
	...
	if CountryCode != "" {
		...
		rows = db.Table("page p").Select("p.*").
			Joins("inner join page_target_platform ptp on ptp.page_id=p.id").
			Joins("join country c on c.alpha2code = ?", CountryCode).
			Joins("join page_country pc on pc.country_id = c.id and pc.page_id = p.id").
			Where("p.is_disabled=false and p.deleted_by_user_id is null and ptp.target_platform=(select id from publish_platform pp where lower(platform)=lower(?))", DeviceName).Order("ptp.page_order_number asc").Find(&pageDetails).RowsAffected
		...
		if err := db.Raw(`
					...
					inner join page_target_platform ptp on
						ptp.page_id = p.id
					
					join country c on c.alpha2code = ? 
						
					join page_country pc on pc.country_id = c.id and pc.page_id = p.id

					where
						(p.is_disabled = false
							and p.deleted_by_user_id is null
							and ptp.target_platform =(
							select
								id
							from
								publish_platform pp
							where
								lower(pp.platform)= lower(?)))
					order by
						ptp.page_order_number asc
				`, CountryCode, DeviceName).Limit(changedlimit).Offset(offset).Find(&pageDetails).Error; err != nil {
			l.JSON(c, http.StatusInternalServerError, serverError)
			return
		}
		...
		if err := db.Raw(`
					...
					order by
						ptp.page_order_number asc
				`, CountryCode, DeviceName).Find(&myplaylistpagedetials).Error; err != nil {
			l.JSON(c, http.StatusInternalServerError, serverError)
			return
		}
		...
		if err := db.Raw(`
					...
					order by
						ptp.page_order_number asc
				`, DeviceName).Limit(changedlimit).Offset(offset).Find(&pageDetails).Error; err != nil {
			l.JSON(c, http.StatusInternalServerError, serverError)
			return
		}
		...
		if err := db.Raw(`
					...
					order by
						ptp.page_order_number asc
				`, DeviceName).Find(&myplaylistpagedetials).Error; err != nil {
			l.JSON(c, http.StatusInternalServerError, serverError)
			return
		}
	}

	// Table("page p").Select("p.*").Joins("inner join page_target_platform ptp on ptp.page_id=p.id").
	// 	Where("p.is_disabled=false and p.page_type = 16 and p.deleted_by_user_id is null and ptp.target_platform=(select id from publish_platform pp where lower(platform)=lower(?))", DeviceName).
	// 	Order("ptp.page_order_number asc").Find(&myplaylistpagedetials).Error; err != nil {
	// 	l.JSON(c, http.StatusInternalServerError, serverError)
	// 	return
	// }
	...
	for _, details := range pageDetails {
		...
		var pagePlaylistCount int
		var pageSliderCount int
		...
		db.Debug().Raw("select count(*) from page_slider ps join slider s on s.id = ps.slider_id where ps.page_id = ? and s.is_disabled is false ", details.Id).Count(&pageSliderCount)
		db.Debug().Raw("select count(*) from page_playlist pp join playlist p on p.id = pp.playlist_id where page_id = ? and p.is_disabled  is false", details.Id).Count(&pagePlaylistCount)
		...
		if SliderCount > 0 || PlaylistCount > 0 || details.EnglishTitle == "My Playlist" || details.ArabicTitle == "قائمتي" {
			...
			if pagePlaylistCount == 0 && pageSliderCount == 0 {
				if details.EnglishTitle != "My Playlist" {
					continue
				}
			}
			var menuPage MenuPage
			...
			if menuPage.ID != myplaylistpagedetials.PageKey {
	...
	if myplaylistpagedetials.HasMobileMenu == true {
		myplaylistpage.Imagery.MobileMenu = os.Getenv("IMAGES") + strings.ToLower(myplaylistpagedetials.Id) + "/mobile-menu"
		myplaylistpage.Imagery.MobilePosterImage = os.Getenv("IMAGES") + strings.ToLower(myplaylistpagedetials.Id) + "/menu-poster-image"
		myplaylistpage.Imagery.MobileMenuPosterImage = os.Getenv("IMAGES") + strings.ToLower(myplaylistpagedetials.Id) + "/mobile-menu-poster-image"
	} else {
	...
	if DeviceName == "appletv" || DeviceName == "AppleTV" {

		var (
			AppleTVsettings Page
			AppleTVsearch   Page
		)

		db.Table("page").Where("page_key = ?", 67).Find(&AppleTVsettings)
		db.Table("page").Where("page_key = ?", 89).Find(&AppleTVsearch)

		if language == "en" {

			menuPageDetails = append(menuPageDetails, MenuPage{
				ID:             AppleTVsettings.PageKey,
				FriendlyUrl:    AppleTVsettings.EnglishPageFriendlyUrl,
				SeoDescription: AppleTVsettings.EnglishMetaDescription,
				Title:          AppleTVsettings.EnglishTitle,
				Type:           "Settings",
			...
			menuPageDetails = append(menuPageDetails, MenuPage{
				ID:             AppleTVsearch.PageKey,
				FriendlyUrl:    AppleTVsearch.EnglishPageFriendlyUrl,
				SeoDescription: AppleTVsearch.EnglishMetaDescription,
				Title:          AppleTVsearch.EnglishTitle,
				Type:           "VOD",
				Imagery: ImageryDetails{
					MobileMenu:            "",
					MobilePosterImage:     "",
			...
			menuPageDetails = append(menuPageDetails, MenuPage{
				ID:             AppleTVsettings.PageKey,
				FriendlyUrl:    AppleTVsettings.ArabicPageFriendlyUrl,
				SeoDescription: AppleTVsettings.ArabicMetaDescription,
				Title:          AppleTVsettings.ArabicTitle,
				Type:           "Settings",
				Imagery: ImageryDetails{
					MobileMenu:            "",
					MobilePosterImage:     "",
			...
			menuPageDetails = append(menuPageDetails, MenuPage{
				ID:             AppleTVsearch.PageKey,
				FriendlyUrl:    AppleTVsearch.ArabicPageFriendlyUrl,
				SeoDescription: AppleTVsearch.ArabicMetaDescription,
				Title:          AppleTVsearch.ArabicTitle,
				Type:           "VOD",
				Imagery: ImageryDetails{
					MobileMenu:            "",
					MobilePosterImage:     "",
...
func (hs *HandlerService) GetTopMenuDetails(c *gin.Context) {
	...
	if c.Request.URL.Query()["limit"] != nil {
		...
		l.JSON(c, http.StatusOK, menuDetails.Details)
		return
	} else {
		...
		// fmt.Println(seriesresult, moviesresult, programsresult, livetvresult, playsresult, "llllll")
...
func PlaylistItemContentsJ(playlistId string, c *gin.Context) (postgres.Jsonb, error) {
	db := c.MustGet("FCDB").(*gorm.DB)
	var playlistResponse FeaturedPlaylistsResponse
	db.Debug().Raw("select jsonb_agg(coalesce(pi2.multi_tier_content_id, pi2.one_tier_content_id,case when pi2.season_id is not null then pic.content_id end))as content from playlist p left join playlist_item pi2 on pi2.playlist_id = p.id left join playlist_item_content pic on pic.playlist_item_id = pi2.id where p.id =? and p.is_disabled='false' group by p.id", playlistId).Find(&playlistResponse)
	return playlistResponse.Content, nil
}
func PlaylistPages(playlistId, language string, pageContents chan []PageContent, c *gin.Context) {
	...
	for _, page := range pageContent {
		var imageryDetails ImageryDetails
		imageryDetails.MobileMenu = os.Getenv("IMAGES") + strings.ToLower(page.Key) + "/mobile-menu"
		imageryDetails.MobilePosterImage = os.Getenv("IMAGES") + strings.ToLower(page.Key) + "/menu-poster-image"
		imageryDetails.MobileMenuPosterImage = os.Getenv("IMAGES") + strings.ToLower(page.Key) + "/mobile-menu-poster-image"
