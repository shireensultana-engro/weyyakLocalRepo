...
func (hs *HandlerService) SliderAvailabilityUpdate(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&sliderAvailability); err != nil {
		l.JSON(c, http.StatusBadRequest, gin.H{"message": err.Error(), "data": http.StatusBadRequest})
		return
	}
	sliderurlId := c.Param("id")
	sliderId := sliderAvailability.Id
	if sliderurlId != sliderId {
		l.JSON(c, http.StatusBadRequest, gin.H{"message": "Please provide valid slider Id.", "Status": http.StatusBadRequest})
		return
	}
	var totalcount int
	...
	if totalcount < 1 {
		l.JSON(c, http.StatusBadRequest, gin.H{"code": "error_slider_not_found", "message": "The specified condition was not met for 'Id'.", "Status": http.StatusBadRequest})
		return
	} else {
...
func (hs *HandlerService) GetSliderTypes(c *gin.Context) {
	...
	var errorresponse = common.ServerErrorResponse()
	fields := "id, name"
	if data := db.Debug().Table("slider_types").Select(fields).Order("id").Scan(&SliderType).Error; data != nil {
		l.JSON(c, http.StatusBadRequest, errorresponse)
		return
	}
	go common.RedisFlush(c)
	go common.Pagekey()
	...
	l.JSON(c, http.StatusOK, gin.H{"data": SliderType})
}
...
func (hs *HandlerService) GetSliderList(c *gin.Context) {
	...
	type RedisResponse struct {
		...
		Finalsliderlist []SliderList   `json:"finalsliderlist"`
	}
	...
	// redisKey := os.Getenv("REDIS_CONTENT_KEY") + "_CreateSlider" + "*offset_" + c.Request.URL.Query()["offset"][0] + "_limit_" + c.Request.URL.Query()["limit"][0]
	...
	// 		l.JSON(c, http.StatusOK, gin.H{"pagination": finalredisResponse.Pagination, "data": finalredisResponse.Finalsliderlist})
	// 		return
	// 	}
	// }

	//::previously vj249(12 months ago change) -> wrong type cast as bool but now hari changed type to boolean in string_agg -> ps.order groupby where added
	// rawquery := "select s.id,s.name,s.is_disabled,s.scheduling_end_date,string_agg(distinct p.english_title::text, ',')::varchar as found_in,string_agg(distinct case when ps.order = 0 then 'true' else 'false' end,',')::boolean as is_default_for_any_page,case when count(distinct pp.id) = 10 then 'All Platforms' else string_agg(distinct pp.platform :: varchar, ',') end as available_on,case when count(distinct c.english_name)= 248 then 'All Regions' else string_agg(distinct c.english_name ::varchar, ',' order by c.english_name asc ) end as region,case when count(distinct sc.country_id)<5 or count(distinct sc.country_id)= 248 then 'false' else 'true' end as has_more_regions from slider s left join page_slider ps on ps.slider_id =s.id left join page p on p.id =ps.page_id join slider_country sc on s.id = sc.slider_id join country c on sc.country_id = c.id and sc.slider_id = s.id join slider_target_platform stp on s.id = stp.slider_id join publish_platform pp on stp.target_platform = pp.id where s.deleted_by_user_id is null and s.name != ''"
	...
	rawquery += "  group by s.id, ps.order order by s.modified_at DESC "

	var sliderlist, finalsliderlist, totalCount []SliderList

	if data := db.Debug().Raw(rawquery).Limit(limit).Offset(offset).Find(&sliderlist).Error; data != nil {
		l.JSON(c, http.StatusInternalServerError, errorresponse)
		return
	}
	...
	for _, sliderResponse := range sliderlist {
		...
		// SchedulingEndDate := strings.Split(sliderResponse.SchedulingEndDate, "T")
		// if SchedulingEndDate[0] != "" {
		// 	sliderResponse.SchedulingEndDate = SchedulingEndDate[0]
		// }
	...
	// 	Pagination:      pages,
	// 	Finalsliderlist: finalsliderlist,
	// })
	...
	l.JSON(c, http.StatusOK, gin.H{"pagination": pages, "data": finalsliderlist})
}
...
func (hs *HandlerService) GetSliderDetailsById(c *gin.Context) {
	...
	l.JSON(c, http.StatusOK, gin.H{"data": Slider})
	return
}
...
func (hs *HandlerService) UpdateSlider(c *gin.Context) {
	...
	if request.BlackAreaPlaylistId != "" {
		var count int
		db.Debug().Table("playlist p").Where("p.id =? and ((p.scheduling_start_date <= ? and p.scheduling_end_date >=?) or (p.scheduling_start_date IS NULL and p.scheduling_end_date IS NULL))", request.BlackAreaPlaylistId, request.SchedulingStartDate, request.SchedulingEndDate).Count(&count)
		if count < 1 {
			errorFlag = true
	...
	fmt.Println("playlist1111111111111111")
	...
	if request.GreenAreaPlaylistId != "" {
		var greencount int
		db.Table("playlist p").Where("p.id =? and ((p.scheduling_start_date <= ? and p.scheduling_end_date >=?) or (p.scheduling_start_date IS NULL and p.scheduling_end_date IS NULL))", request.GreenAreaPlaylistId, request.SchedulingStartDate, request.SchedulingEndDate).Count(&greencount)
		if greencount < 1 {
			errorFlag = true
	...
	if request.RedAreaPlaylistId != "" {
		var redcount int
		db.Table("playlist p").Where("p.id =? and ((p.scheduling_start_date <= ? and p.scheduling_end_date >=?) or (p.scheduling_start_date IS NULL and p.scheduling_end_date IS NULL))", request.RedAreaPlaylistId, request.SchedulingStartDate, request.SchedulingEndDate).Count(&redcount)
		if redcount > 0 {
			var totalCount int
			fmt.Println("---------------------------")

			db.Debug().Raw("select count(pi2.id) from playlist_item pi2 left join playlist_item_content pic on pic.playlist_item_id = pi2.id where pi2.playlist_id =?", request.RedAreaPlaylistId).Count(&totalCount)
			if totalCount < 7 {
				errorFlag = true
	...
	if sliderId != "" {
		...
		// slider.MoreButton = request.MoreButton
		// slider.PlayWatchTrailer = request.PlayWatchTrailer
		...
		if err := tx.Debug().Table("slider").Where("id=?", c.Param("id")).Updates(map[string]interface{}{
			"name":                   slider.Name,
			"scheduling_end_date":    slider.SchedulingEndDate,
			"scheduling_start_date":  slider.SchedulingStartDate,
			"type":                   slider.Type,
			"modified_at":            slider.ModifiedAt,
			"black_area_playlist_id": slider.BlackAreaPlaylistId,
			"green_area_playlist_id": slider.GreenAreaPlaylistId,
			"red_area_playlist_id":   slider.RedAreaPlaylistId,
			"play_watch_trailer":     request.PlayWatchTrailer,
			"more_button":            request.MoreButton,
		}).Error; err != nil {
			l.JSON(c, http.StatusInternalServerError, serverError)
			return
		}
		...
		for _, pageId := range request.PagesIds {
			...
			if err := db.Debug().Table("page").Select("page_key").Where("id=?", pageId).Find(&pageKey).Error; err != nil {
				l.JSON(c, http.StatusInternalServerError, serverError)
				return
			}
...
func (hs *HandlerService) CreateSlider(c *gin.Context) {
	...
	fmt.Println("requestttttttttttttttttt", request)
	if err := c.ShouldBindJSON(&request); err != nil {
	...
	var errorFlag bool
	fmt.Println("errorflag", errorFlag)
	errorFlag = false
	if request.Regions == nil || request.PublishingPlatforms == nil {
		l.JSON(c, http.StatusInternalServerError, serverError)
		return
	...
	if len(request.Name) == 0 {
		errorFlag = true
		nameError = common.NameError{Code: "error_slider_name_not_unique", Description: "Slider name is empty"}
	}

	db.Debug().Table("slider").Where("name = ?", request.Name).Count(&nameUniqueCheck)
	// if nameUniqueCheck > 0 {
	// 	errorFlag = true
	// 	nameError = common.NameError{Code: "error_slider_name_not_unique", Description: "Slider with specified 'Name' of " + request.Name + " already exists."}
	// }
	...
	//Notification Based Content
	// var Notifications []SliderNotificationError
	if request.BlackAreaPlaylistId != "" {
		var playlist SliderPlaylistDetails
		if err := db.Debug().Table("playlist").Where("id=?", request.BlackAreaPlaylistId).Find(&playlist).Error; err != nil {
			l.JSON(c, http.StatusInternalServerError, serverError)
			return
		}
		countError := GetPlaylistContentsErrors(request.BlackAreaPlaylistId, request.SchedulingStartDate, request.SchedulingEndDate, request.Name, common.BLACK_AREA_PLAYLIST_CONUT, c, playlist)
		if countError.Message != "" {
			errorFlag = true
			blackerror = common.BlackAreaPlaylistError{Code: countError.Code, Description: countError.Message}
		}
		if countError.Message == "" {
			scheduleError := CheckPlaylistSheduleDate(request.BlackAreaPlaylistId, request.SchedulingStartDate, request.SchedulingEndDate, request.Name, c, playlist)
			if scheduleError.Message != "" {
				errorFlag = true
				fmt.Println(scheduleError.Message)
				blackerror = common.BlackAreaPlaylistError{Code: scheduleError.Code, Description: scheduleError.Message}
			}
			disableError := CheckPlaylistDisabledOrNot(request.BlackAreaPlaylistId, request.Name, c, playlist)
	...
	if request.GreenAreaPlaylistId != "" {
		var playlist SliderPlaylistDetails
		if err := db.Debug().Table("playlist").Where("id=?", request.GreenAreaPlaylistId).Find(&playlist).Error; err != nil {
			/* l.JSON(c, http.StatusInternalServerError, serverError)
			return */
		}
		countError := GetPlaylistContentsErrors(request.GreenAreaPlaylistId, request.SchedulingStartDate, request.SchedulingEndDate, request.Name, common.GREEN_AREA_PLAYLIST_CONUT, c, playlist)
		if countError.Message != "" {
			errorFlag = true
			greenerror = common.GreenAreaPlaylistError{Code: countError.Code, Description: countError.Message}
		}
		if countError.Message == "" {
			scheduleError := CheckPlaylistSheduleDate(request.GreenAreaPlaylistId, request.SchedulingStartDate, request.SchedulingEndDate, request.Name, c, playlist)
			if scheduleError.Message != "" {
				errorFlag = true
				greenerror = common.GreenAreaPlaylistError{Code: countError.Code, Description: countError.Message}
			}
			disableError := CheckPlaylistDisabledOrNot(request.GreenAreaPlaylistId, request.Name, c, playlist)
			if disableError.Message != "" {
				errorFlag = true
				greenerror = common.GreenAreaPlaylistError{Code: countError.Code, Description: countError.Message}
			}
	...
	if request.RedAreaPlaylistId != "" {
		var playlist SliderPlaylistDetails
		if err := db.Debug().Table("playlist").Where("id=?", request.RedAreaPlaylistId).Find(&playlist).Error; err != nil {
			/* l.JSON(c, http.StatusInternalServerError, serverError)
			return */
		}
		countError := GetPlaylistContentsErrors(request.RedAreaPlaylistId, request.SchedulingStartDate, request.SchedulingEndDate, request.Name, common.RED_AREA_PLAYLIST_CONUT, c, playlist)
		if countError.Message != "" {
			errorFlag = true
			rederror = common.RedAreaPlaylistError{Code: countError.Code, Description: countError.Message}
		}
		if countError.Message == "" {
			scheduleError := CheckPlaylistSheduleDate(request.RedAreaPlaylistId, request.SchedulingStartDate, request.SchedulingEndDate, request.Name, c, playlist)
			if scheduleError.Message != "" {
				errorFlag = true
				rederror = common.RedAreaPlaylistError{Code: countError.Code, Description: scheduleError.Message}
			}
			disableError := CheckPlaylistDisabledOrNot(request.RedAreaPlaylistId, request.Name, c, playlist)
			if disableError.Message != "" {
				errorFlag = true
				rederror = common.RedAreaPlaylistError{Code: countError.Code, Description: disableError.Message}
			}
		}
	}
	//Notification Based Content
	...
	type SliderKey struct {
		Key                 int `json:"key"`
		ThirdPartySliderKey int `json:"third_party_slider_key"`
	}
	var sliderKey SliderKey
	if err := db.Debug().Table("slider").Select("max(slider_key)+1 as key,max(third_party_slider_key) as third_party_slider_key").Find(&sliderKey).Error; err != nil {
		l.JSON(c, http.StatusInternalServerError, serverError)
		return
	}
	// for creating third party slider key
	slider.ThirdPartySliderKey = sliderKey.ThirdPartySliderKey + 1
	slider.CreatedAt = time.Now()
	slider.ModifiedAt = time.Now()
	// for removing sync below line is replaced with slider.SliderKey = sliderKey.Key
	//slider.SliderKey = request.SliderKey
	slider.SliderKey = sliderKey.Key
	slider.PlayWatchTrailer = request.PlayWatchTrailer
	slider.MoreButton = request.MoreButton
	...
	err = common.DeleteRedisDataWithKey("GOAPIQA_CreateSlider*")
	if err != nil {
		l.JSON(c, http.StatusInternalServerError, serverError)
		return
	}
...
func (hs *HandlerService) GetSliderPreviewlayouts(c *gin.Context) {
	...
	if data := db.Debug().Table("preview_layout").Select("slider_type , platform,id, preview_image_key").Find(&result).Error; data != nil {
		l.JSON(c, http.StatusBadRequest, common.ServerErrorResponse())
		return
	}
	...
	l.JSON(c, http.StatusOK, gin.H{"data": newarr})
}
...
func (hs *HandlerService) GetSlidersBasedOnSearchInPage(c *gin.Context) {
	...
	if searchText != "" {
		rawquery += " and name ilike '%" + searchText + "%'"
	}
	rawquery += " group by id"
	var sliderlist, totalCount []PageSummary

	if data := db.Debug().Raw(rawquery).Order("(case when name = '" + searchText + "' or name = '" + strings.Title(searchText) + "' or name = '" + strings.ToLower(searchText) + "' then 1 else 2 end), name").Limit(limit).Offset(offset).Scan(&sliderlist).Error; data != nil {
		l.JSON(c, http.StatusInternalServerError, serverError)
		return
	}
	if errCount := db.Debug().Raw(rawquery).Scan(&totalCount).Error; errCount != nil {
	...
	l.JSON(c, http.StatusOK, gin.H{"pagination": pages, "data": sliderlist})
}
...
func (hs *HandlerService) GetSliderNotifications(c *gin.Context) {
	...
	l.JSON(c, http.StatusOK, Notifications)
	return
}
...
func (hs *HandlerService) GetallRegionsBasedOnSliderId(c *gin.Context) {
	...
	l.JSON(c, http.StatusOK, gin.H{"data": sliderRegions})
}
