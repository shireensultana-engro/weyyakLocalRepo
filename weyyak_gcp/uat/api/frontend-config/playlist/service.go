...
func (hs *HandlerService) GetAllPlaylistAndPlaylistsBySearchText(c *gin.Context) {
	...
	l.JSON(c, http.StatusOK, gin.H{"pagination": pages, "data": playlists})
}
...
func (hs *HandlerService) GetSearchFiltersForPlaylists(c *gin.Context) {
	...
	l.JSON(c, http.StatusOK, gin.H{"data": sourceitemtypes})
}
...
func (hs *HandlerService) DeletePlaylistByPlaylistId(c *gin.Context) {
	...
	if err := db.Debug().Table("playlist").Where("id=? and deleted_by_user_id is null ", playlistid).Find(&playlist).Error; err != nil {
		l.JSON(c, http.StatusBadRequest, gin.H{"message": "Record does not exist.Please provide valid playlist Id.", "Status": http.StatusBadRequest})
		return
	}
...
func (hs *HandlerService) DisablePlaylistByPlaylistId(c *gin.Context) {
	...
	if playlistidRrl != playlistid {
		l.JSON(c, http.StatusBadRequest, gin.H{"message": "Please provide valid playlist Id.", "Status": http.StatusBadRequest})
		return
	}
	var totalcount int
	...
	playlist.ModifiedAt = time.Now()
	if totalcount < 1 {
		l.JSON(c, http.StatusBadRequest, gin.H{"code": "error_playlist_not_found", "message": "The specified condition was not met for 'Id'.", "Status": http.StatusBadRequest})
		return
	} else {
		if err := db.Debug().Table("playlist").Where("id=?", playlistid).Update(&playlist).Error; err != nil {
			l.JSON(c, http.StatusInternalServerError, gin.H{"error": "server_error", "description": "Server error.", "code": "error_server_error", "requestId": randstr.String(32)})
...
func (hs *HandlerService) GetPlaylistDetailsByPlaylistId(c *gin.Context) {
	...
	l.JSON(c, http.StatusOK, gin.H{"data": playlist})
	return
}
...
func (hs *HandlerService) GetSourceitemsBySourceFields(c *gin.Context) {
	...
	if searchText == "" || searchFilter == 0 {
		l.JSON(c, http.StatusBadRequest, gin.H{"message": "Empty Parameters", "Status": http.StatusBadRequest})
		return
	}
	...
	l.JSON(c, http.StatusOK, gin.H{"pagination": pages, "data": sourceitemdata})
}
...
func (hs *HandlerService) getSourceItem(c *gin.Context) {
	...
	l.JSON(c, http.StatusOK, gin.H{"data": sourceItem})
}
...
func (hs *HandlerService) GetPlaylistItemBasedOnPlaylistItemTypeAndItemTypeID(c *gin.Context) {
	...
	l.JSON(c, http.StatusOK, gin.H{"pagination": pages, "data": playListItem})
}
...
func (hs *HandlerService) CreateAndUpdatePlaylist(c *gin.Context) {
	...
	if playlistid != "" {
		...
		if err := fcdb.Debug().Where("id=? ", playlistid).Find(&playlist).Error; err != nil {
			l.JSON(c, http.StatusBadRequest, gin.H{"message": "Invalid Request", "Status": http.StatusBadRequest})
			return
		}
		...
		if input.SchedulingStartDate == nil {
			if err := fctx.Debug().Table("playlist").Where("id=?", playlistid).Update("scheduling_start_date", nil).Error; err != nil {
				l.JSON(c, http.StatusInternalServerError, errorresponse)
				return
			}
		}

		if input.SchedulingEndDate == nil {
			if err := fctx.Debug().Table("playlist").Where("id=?", playlistid).Update("scheduling_end_date", nil).Error; err != nil {
				l.JSON(c, http.StatusInternalServerError, errorresponse)
				return
			}
		}
		...
		/*update Page IDs Start */
		fmt.Println("input.PagesIds------------->", input.PagesIds)
		...
		} else if len(input.PagesIds) == 0 {

			var pageplay PagePlaylist
			if err := fcdb.Debug().Table("page_playlist").Where("playlist_id = ?", playlistid).Delete(&pageplay).Error; err != nil {
				l.JSON(c, http.StatusInternalServerError, errorresponse)
				return
			}
		...
		go common.GetMenu()
		go common.Contenttype()
		l.JSON(c, http.StatusOK, gin.H{"data": finPlaylistId})
		return
	} else {
