...
func (hs *HandlerService) PageOrder(c *gin.Context) {
	...
	l.JSON(c, http.StatusOK, gin.H{"data": final})
	return
}
...
func (hs *HandlerService) DeletePageDetailsById(c *gin.Context) {
	...
	if totalCount < 1 {
		...
		l.JSON(c, http.StatusOK, gin.H{"message": "Record deleted successfully"})
	}
}
...
func (hs *HandlerService) PageAvailabilityUpdate(c *gin.Context) {
	...
	if pageIdUrl != pageUpdate.Id {
		l.JSON(c, http.StatusBadRequest, gin.H{"message": "Please provide valid page Id.", "Status": http.StatusBadRequest})
		return
	}
...
func (hs *HandlerService) GetPagesListBySearchText(c *gin.Context) {
	...
	l.JSON(c, http.StatusOK, gin.H{"pagination": pages, "data": pagelists})
}
...
func (hs *HandlerService) GetAllPagesListBySearchText(c *gin.Context) {
	...
	if data := db.Debug().Raw(rawquery).Limit(limit).Offset(offset).Scan(&pagelist).Error; data != nil {
		l.JSON(c, http.StatusInternalServerError, FinalErrorResponse{Error: "server_error", Description: "Server error.", Code: "error_server_error", RequestId: randstr.String(32)})
		return
	}
	var platformDetails []PlanNames
	for _, pltid := range pagelist {
...
func (hs *HandlerService) UpdatePageOrder(c *gin.Context) {
	...
	if errresult := c.ShouldBindJSON(&response); errresult != nil {
		l.JSON(c, http.StatusBadRequest, gin.H{"message": errresult.Error(), "status": http.StatusBadRequest})
		return
	}
...
func (hs *HandlerService) CreateOrUpdatePageDetails(c *gin.Context) {
	...
	if c.Param("id") == "" {
		...
		l.JSON(c, http.StatusOK, gin.H{"data": finPageId})
		return
		...
		for _, req := range pageRequest.PublishingPlatforms {
			...
			if !(exist) {
				...
				// 	l.JSON(c, http.StatusInternalServerError, new)
				// 	return
				// }
		...
		// 	fmt.Println(updateError, ";;;;;;;;")
		// 	l.JSON(c, http.StatusInternalServerError, updateError)
		// 	return
		// }
...
func (hs *HandlerService) GetallPageRegionsByPageId(c *gin.Context) {
	...
	l.JSON(c, http.StatusOK, gin.H{"data": pageRegions})
}
...
func PageFileUploadGcp(request NonTextualData, pageId string) {
	...
	for _, item := range newarr {
		...
		if err != nil {
			...
			source = pageId + "/" + filetrims[len(filetrims)-1]
			destination = pageId + "/" + filetrims[len(filetrims)-1]
			srcObject = client.Bucket(bucketName).Object(source)
		...
		errorr := SizeUploadFileToGcp(ctx, client, filetrim[0], pageId, url)
		if errorr != nil {
			fmt.Println("error in uploading size upload", errorr)
		}
		if ferr == nil {
