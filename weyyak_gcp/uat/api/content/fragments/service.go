...
func (hs *HandlerService) ContentsPagesPlaylistsSlidersDigitalRightsExceeded(c *gin.Context) {
	...
	if err := fdb.Debug().Raw("select distinct cf.content_id from content_fragment cf where (cf.rights_end_date  between now()  - interval  '2090 DAY' and now()) or (cf.rights_start_date > now()) ").Find(&contentids).Error; err != nil {
	...
	for _, val := range contentids {
		...
		fdb.Debug().Exec("DELETE FROM content_fragment where content_id=?", val.ContentId)
	...
	l.JSON(c, http.StatusOK, gin.H{"message": "Done"})
}
...
func CreateContentFragment(contentId string, c *gin.Context) {
	...
	for _, value := range contentDetails {
		...
		contentFragmentDetails.FriendlyUrl = strings.ToLower(value.FriendlyUrl)
		contentFragmentDetails.ProductionYear = value.ProductionYear
		contentFragmentDetails.SchedulingDateTime = value.SchedulingDateTime.String()
		for i := 1; i <= 2; i++ {
			fmt.Println(i)
			if i == 1 {
				lang = "en"
