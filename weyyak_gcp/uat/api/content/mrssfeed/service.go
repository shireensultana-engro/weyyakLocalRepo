...
// 	ax := "https://content-uat.weyyak.com/"
...
func (hs *HandlerService) mrssfeed(c *gin.Context) {
	...
	for _, val := range contentdetails {
		...
		// imagery
		if val.SeasonId != "" {
			temp.Thumbnail.URL = "https://content-uat.weyyak.com/" + val.Id + "/" + val.SeasonId + "/" + val.EpisodeId + "/poster-image"

		} else {
			temp.Thumbnail.URL = "https://content-uat.weyyak.com/" + val.Id + "/poster-image"
		}
		for i := 0; i < 2; i++ {
			var temps Content
			if i == 0 {
