...
func (hs *ContentService) CreateOnetierContent(c *gin.Context) {
	...
	if err != nil {
		l.JSON(c, http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	...
	if err := db.Debug().Create(&content).Error; err != nil {
		l.JSON(c, http.StatusBadRequest, gin.H{"message": err.Error(), "data": nil})
		return
	} else {
		...
		if request.TextualData.ContentVariances != nil {
			for j, variance := range request.TextualData.ContentVariances {
				...
				// playbackItem.YoutubeVideoId = variance.YoutubeVideoId
		...
		l.JSON(c, http.StatusOK, gin.H{"message": "Content Created Successfully.", "status": http.StatusOK})
		return
	}
}
...
func (hs *ContentService) GetMultitierContentDetails(c *gin.Context) {
	...
	if err := db.Debug().Table("content_genre cg").Select("g.id as genre_id,json_agg(sg.id   order by cs.order)::varchar as subgenres_id,json_agg(cs.order  order by cs.order)::varchar as sub_genre_order,cg.id").Joins("join content_subgenre cs on cs.content_genre_id = cg.id join genre g on g.id=cg.genre_id join subgenre sg on sg.id=cs.subgenre_id join content c on c.id = cg.content_id").Where("cg.content_id = ?", c.Param("result")).Group("sg.id,g.id,cg.id").Order("cg.order").Find(&genres).Error; err != nil {
		l.JSON(c, http.StatusInternalServerError, serverError)
		return
	}
	...
	l.JSON(c, http.StatusOK, gin.H{"data": multitierContentDetails})
	return
}
...
func (hs *ContentService) CreateSeason(c *gin.Context) {
	...
	if int32(request.Rights.DigitalRightsType) != common.ContentRightsTypes("Avod") {
		if len(request.Rights.SubscriptionPlans) > 1 {
			l.JSON(c, http.StatusBadRequest, common.ServerError{
				Error:       "Multiple Subscription Plans not allowed",
				Description: "If multiple subscription plans have been assigned to the season content id " + *request.ContentId + ", only one plan can be selected",
				Code:        "",
	...
	if request.Cast == nil {
		...
		if request.Cast.MainActorId == nil {
			errorFlag = true
			casterror = common.CastError{Code: "NotEmptyValidator", Description: "'Main Actor' should not be empty."}
		}
		if request.Cast.MainActressId == nil {
			errorFlag = true
			casterror = common.CastError{Code: "NotEmptyValidator", Description: "'Main Actress' should not be empty."}
		}
	}
	...
	var seasonContentErr common.SeasonGenresError
	if len(request.SeasonGenres) < 2 {
		errorFlag = true
		seasonContentErr = common.SeasonGenresError{Code: "error_multitiercontentGener_not_found", Description: "From (2) to (6) genre(s) are required in 'Content Genres' field in Series"}
	}
	...
	if seasonContentErr.Code != "" {
		invalid.SeasonGenresError = seasonContentErr
	}
	...
	for _, trailerData := range *request.VarianceTrailers {
	...
	if c.Param("id") == "" && request.SeasonId == "" {
		...
		var aboutTheContent AboutTheContentInfoUpdate
		...
		ProductionYearWP, err := strconv.Atoi(request.AboutTheContent.ProductionYear)
		if err != nil {
		...
		aboutTheContent.ProductionYear = ProductionYear
		...
		if err := db.Debug().Table("about_the_content_info").Create(&aboutTheContent).Error; err != nil {
			l.JSON(c, http.StatusInternalServerError, serverError)
			return
		}
		var productionCountry ProductionCountry
		...
		// for fetching max third_party_season_key
		var thirdparty Season
		if err := tx.Debug().Select("max(third_party_season_key) as third_party_season_key").Find(&thirdparty).Error; err != nil {
			l.JSON(c, http.StatusInternalServerError, serverError)
			return
		}
		// for third party season key creation (max(third_party_season_key) + 1)
		season.ThirdPartySeasonKey = thirdparty.ThirdPartySeasonKey + 1
		//	season.SeasonKey = request.SeasonKey // for removing sync this line is commented and above line is uncommented
		...
		//season.Id = request.SecondarySeasonId // for removing sync this line is commented
		...
		// if request.NonTextualData.PosterImage != "" && request.NonTextualData.OverlayPosterImage != "" && request.NonTextualData.DetailsBackground != "" && request.NonTextualData.MobileDetailsBackground != "" && request.NonTextualData.SeasonLogo != "" {
		...
		if len(*request.VarianceTrailers) != 0 {
			for i, trailerrange := range *request.VarianceTrailers {
				if trailerrange.VideoTrailerId != "" {
					...
					// for removing sync below if condition is commented
		...
		aboutTheContent.IntroStart = request.AboutTheContent.IntroStart
		aboutTheContent.OutroStart = request.AboutTheContent.OutroStart
		if err := tx.Debug().Table("about_the_content_info").Create(&aboutTheContent).Error; err != nil {
			l.JSON(c, http.StatusInternalServerError, serverError)
			return
		}
		var productionCountry ProductionCountry
		...
		// for third party season key creation (max(third_party_season_key) + 1)
		season.ThirdPartySeasonKey = thirdparty.ThirdPartySeasonKey + 1
		//season.SeasonKey = request.SeasonKey // for removing sync this line is commented and below line is uncommented
		season.SeasonKey = seasonKey.SeasonKey + 1
		fmt.Println(request.ContentId, "request content id is")
		// for creating old seaesons take season id and user id from request body
		//season.Id = request.SecondarySeasonId // for removing sync this line is commented
		...
		if request.NonTextualData.PosterImage == "" && request.NonTextualData.OverlayPosterImage == "" && request.NonTextualData.DubbingScript == "" && request.NonTextualData.MobileDetailsBackground == "" && request.NonTextualData.SeasonLogo == "" {
			season.HasPosterImage = "false"
			season.HasOverlayPosterImage = "false"
			season.HasDetailsBackground = "false"
			season.HasMobileDetailsBackground = "false"
		...
		if request.SeasonGenres != nil {
			...
			// if len(request.SeasonGenres) < 2 || len(request.SeasonGenres) > 6 {
			// 	genrerror = common.GenresError{Code: "error_multitiercontentGener_not_found", Description: "From (2) to (6) genre(s) are required in 'Content Genres' field in Series"}
			// 	l.JSON(c, http.StatusInternalServerError, genrerror)
			// 	return
			// }
		...
		if len(*request.VarianceTrailers) != 0 {
			for i, trailerrange := range *request.VarianceTrailers {
				if trailerrange.VideoTrailerId != "" {
					...
					// if duration == 0 {
					// 	tx.Rollback()
					// 	serverError = common.ServerError{Error: "TrailerId", Description: "Please provide valid Video TrailerId", Code: "", RequestId: randstr.String(32)}
					// 	l.JSON(c, http.StatusInternalServerError, serverError)
					// 	return
					// }
					var varianceTrailer VarianceTrailer
		...
		/* Fragment Creation */
		fmt.Println("sssseasondraftspublishsss", c.Param("status"))
		fdb.Debug().Exec("DELETE FROM content_fragment where content_variance_id=?", seasonId)
		if season.Status == 1 {
			go fragments.CreateContentFragment(season.ContentId, c)
		}
		/* Prepare Redis Cache for all contents */
		...
		go common.GetMenu()
		go common.Contenttype()
		l.JSON(c, http.StatusOK, gin.H{"message": "Season Created Successfully.", "status": http.StatusOK, "id": season.Id})
		return
	} else {
		// update season
		...
		result, err, seasonStatus := UpdateSeasonDetails(request, c, season, 1)
		fmt.Println(err, ".....................", seasonStatus)
		if result == "" {
		...
		// go common.ContentSynching(season.ContentId, c)
		/* Fragment Creation */
		fmt.Println("sssseasondraftssss", c.Param("status"))
		fdb.Debug().Exec("DELETE FROM content_fragment where content_variance_id=?", seasonId)
		if seasonStatus == 1 {
			go fragments.CreateContentFragment(season.ContentId, c)
		}
...
func (hs *ContentService) DraftSeason(c *gin.Context) {
	...
	// if *request.Cast.MainActressId == *request.Cast.MainActorId {
	// 	l.JSON(c, http.StatusBadRequest, common.FinalErrorResponseepisode{
	// 		Error:       "invalid_request",
	// 		Description: "Main Actress and Main Actor are same",
	// 		Code:        "error_validation_failed",
	// 		RequestId:   randstr.String(32),
	// 		Invalid:     invalid})
	// 	return
	...
	if c.Param("id") == "" && request.SeasonId == "" {
		...
		//SeasonLogo
		if request.NonTextualData.SeasonLogo != "" {
			season.HasSeasonLogo = "true"
		} else {
			season.HasSeasonLogo = "false"
		}
		...
		go common.Contenttype()
		fmt.Println("sssseasondraftssss", c.Param("status"))
		fdb.Debug().Exec("DELETE FROM content_fragment where content_variance_id=?", season.Id)
		l.JSON(c, http.StatusOK, gin.H{"message": "Season Created Successfully.", "status": http.StatusOK, "id": season.Id})
		return
	} else {
		if err := tx.Debug().Where("id=?", c.Param("id")).Find(&season).Error; err != nil {
		...
		result, _, _ := UpdateSeasonDetails(request, c, season, 3)
		if result == "" {
			l.JSON(c, http.StatusNotFound, serverError)
			return
		}
		res := map[string]string{
			"id": c.Param("id"),
		...
		go common.GetMenu()
		go common.Contenttype()
		fmt.Println("sssseasondrafts", c.Param("status"))
		fdb.Debug().Exec("DELETE FROM content_fragment where content_variance_id=?", c.Param("id"))
		l.JSON(c, http.StatusOK, gin.H{"data": res})
		return
	}
}
func UpdateSeasonDetails(request CreateSeasonRequestValidation, c *gin.Context, seasonDetails Season, status int) (string, common.ServerError, int) {
	...
	if primaryinfoupdate := tx.Debug().Table("content_primary_info").Where("id=?", seasonDetails.PrimaryInfoId).Update(map[string]interface{}{
		"alternative_title":    request.PrimaryInfo.AlternativeTitle,
		"arabic_title":         request.PrimaryInfo.ArabicTitle,
		"intro_start":          request.IntroStart,
		"notes":                request.PrimaryInfo.Notes,
		"original_title":       request.PrimaryInfo.OriginalTitle,
		"outro_start":          "",
		"transliterated_title": request.PrimaryInfo.TransliteratedTitle,
	...
	fmt.Println("request.AboutTheContent.ProductionYear----------->", request.AboutTheContent.ProductionYear)

	if res := tx.Debug().Table("about_the_content_info").Where("id=?", seasonDetails.AboutTheContentInfoId).Update(map[string]interface{}{
		"original_language":      aboutTheContent.OriginalLanguage,
		"supplier":               aboutTheContent.Supplier,
		"acquisition_department": aboutTheContent.AcquisitionDepartment,
		"english_synopsis":       aboutTheContent.EnglishSynopsis,
		"arabic_synopsis":        aboutTheContent.ArabicSynopsis,
		"production_year":        aboutTheContent.ProductionYear,
		"production_house":       aboutTheContent.ProductionHouse,
		"age_group":              aboutTheContent.AgeGroup,
		"intro_duration":         aboutTheContent.IntroDuration,
		"intro_start":            aboutTheContent.IntroStart,
		"outro_duration":         aboutTheContent.OutroDuration,
		"outro_start":            aboutTheContent.OutroStart,
	}).Error; res != nil {
		// tx.Rollback()
		// l.JSON(c, http.StatusInternalServerError, serverError)
		// return
	...
	// if err := tx.Debug().Table("about_the_content_info").Where("id=?", seasonDetails.AboutTheContentInfoId).Update(&aboutTheContent).Error; err != nil {
	// 	return "", serverError
	// }
	var productionCountry ProductionCountry
	...
	var startDate, endDate, nowDate time.Time
	startDate, _ = time.Parse("2006-01-02T00:00:00.000Z", contentRights.DigitalRightsStartDate)
	endDate, _ = time.Parse("2006-01-02T00:00:00.000Z", contentRights.DigitalRightsEndDate)
	nowDate, _ = time.Parse("2006-01-02T00:00:00.000Z", time.Now().Format("2006-01-02T00:00:00.000Z"))
	fmt.Println(startDate, endDate, nowDate, "ddddatee++++++++")
		...
		for i, trailerrange := range *request.VarianceTrailers {
			if trailerrange.Id == "" {
				if trailerrange.VideoTrailerId != "" {
					...
					// }
					// for removing sync above lines are commented
					varianceTrailer.EnglishTitle = trailerrange.EnglishTitle
					varianceTrailer.ArabicTitle = trailerrange.ArabicTitle
					varianceTrailer.VideoTrailerId = trailerrange.VideoTrailerId
	...
	/*commit changes*/
	if err := tx.Commit().Error; err != nil {
		return "", serverError, seasonStatus

	}
	if status == 1 {
		return "Season updated successfully.", serverError, seasonStatus
	} else {
		return "Season drafted successfully.", serverError, seasonStatus
	}
...
func (hs *ContentService) GetAllContentDetails(c *gin.Context) {
	...
	redisKey := os.Getenv("REDIS_CONTENT_KEY") + "BOApiContent" + "*offset_" + c.Request.URL.Query()["offset"][0] + "_limit_" + c.Request.URL.Query()["limit"][0] + "_content_type_" + contentType
	...
	// if searchText == "" && contentType != "" {
	// 	val, err := common.GetRedisDataWithKey(redisKey)
	// 	if err == nil {
	// 		var (
	// 			redisResponse      RedisCacheResponse
	// 			finalredisResponse Response
	// 			redisErrorResponse RedisErrorResponse
	// 		)
	...
	// 			l.JSON(c, http.StatusOK, gin.H{"pagination": finalredisResponse.Pagination, "data": finalredisResponse.Data})
	// 			return
	// 		}
	// 	}
	// }
	...
	for _, content := range contentDetails {
		...
		if content.Type == 1 {
			...
			for _, details := range varianceDetails {
				if content.Status == 2 {
					...
					if details.Status == 2 {
						...
						if anotherdate != "" && newdates != "" {
							...
							if schedulingforvariance > time.Now().String() && details.SchedulingDateTime != nil && details.Status != 3 {
								status := "Scheduled"
								details.SubStatusName = status
								details.StatusCanBeChanged = true
							} else if schedulingforvariance > time.Now().String() {
								status := "Scheduled"
								details.SubStatusName = status
								details.StatusCanBeChanged = true
							} else if anotherdate <= time.Now().Format("2006-01-02") {
					...
					if anotherdate != "" && newdates != "" {
						if anotherdate <= time.Now().Format("2006-01-02") {
							...
							details.SubStatusName = status
							details.StatusCanBeChanged = false
						} else if scheduledate > time.Now().String() && details.SchedulingDateTime != nil && details.Status != 3 {
							//	status := "Unpublished"
							details.Status = 2
				...
				//commented below code to solve scheduling-->draft issue
				// if details.SchedulingDateTime != nil {
			...
			for _, sDetails := range seasonDetails {
				...
				for _, eDetails := range episodeDetails {
					...
					if sDetails.Status == 2 {
						...
						} else if scheduledate > time.Now().String() && eDetails.Status == 1 {
							status := "Scheduled"
							eDetails.Status = 2
							eDetails.SubStatusName = &status
							eDetails.StatusCanBeChanged = false
							//Substate issue added -> && sDetails.Status != 2
						} else if newdates > time.Now().Format("2006-01-02") && eDetails.Status == 1 && sDetails.Status != 2 {
							status := "Draft"
							eDetails.Status = 2
							eDetails.SubStatusName = &status
							eDetails.StatusCanBeChanged = false
	...
	l.JSON(c, http.StatusOK, finalResult)
	return
}
...
func SizeUploadFileToS3(s *session.Session, fileName string, contentId string, seasonId string) error {
	...
	sizeValue := [20]string{
		...
		"811x811/",
		"900x900/",
		// "2048x670/", Old Detail's page Background
		"1125x240/",
		"1920x1080/", //New for Detail's page Background
		"1125x540/",
	}
	var er error
...
func (hs *ContentService) GetEpisodeDetailsBYepisodeId(c *gin.Context) {
	...
	seasonEpisode.SubscriptionPlanId = episodeDetails.SubscriptionPlanId
	...
	l.JSON(c, http.StatusOK, gin.H{"data": seasonEpisode})
	return
}
...
func (hs *ContentService) PageSyncWithContentId(c *gin.Context) {
	...
	for _, ids := range contentIds {
		...
		if deleteError := fdb.Debug().Where("content_id=?", ids.ContentId).Delete(&deletecontentSync).Error; deleteError != nil {
...
func (hs *ContentService) UploadMenuPosterImageGcp(c *gin.Context) {
	...
	if err != nil {
		...
		fmt.Println("Could not upload file")
		l.JSON(c, http.StatusBadGateway, gin.H{"data": "Could not upload file"})
		return
	}
	fmt.Printf("Image uploaded successfully: %v", fileName)
	go common.RedisFlush(c)
...
func SizeUploadFileToGCP(client *storage.Client, fileName string, contentId string, seasonId string, fileUrl string) error {
	...
	for i := 0; i < len(sizeValue); i++ {
		...
		// https://msapiuat-image.z5.com/crop?width=200&height=200&url=https://content-production.weyyak.com/2b7d164d-eddd-4b6d-9d9c-84df62ccf01b/28e91598-4b43-40aa-89d6-daadb31ef82b/poster-image
...
func SizeUploadFileToGcpEpi(ctx context.Context, client *storage.Client, fileName string, episodeId string, contentId string, seasonId string, fileUrl string) error {
	...
	for i := 0; i < len(sizeValue); i++ {
		...
		width := filetri[0]
		height := filetri[1]
		// https://msapiuat-image.z5.com/crop?width=200&height=200&url=https://content-uat.weyyak.com/2b7d164d-eddd-4b6d-9d9c-84df62ccf01b/28e91598-4b43-40aa-89d6-daadb31ef82b/poster-image
		// Get the URL of the uploaded object in GCS
		url := os.Getenv("RESIZE_IMAGE_URL") + "width=" + width + "&height=" + height + "&url=" + fileUrl
		fmt.Println("urlurlurlurl", url)
		method := "GET"
		...
		if strings.Contains(item, "_") {
			...
			if err != nil {
				...
				if err != nil {
					...
					fmt.Printf("Modified source object does not exist2: %v\n", err)
					fmt.Printf("Source object does not exist2: %v\n", err)

					// Modify the source path if needed
					filetrims := strings.Split(item, "_")
					source = contentId + "/" + seasonId + "/" + TrailerId + "/" + filetrims[0]
					Destination = contentId + "/" + seasonId + "/" + TrailerId + "/" + filetrims[0]
					srcObject = client.Bucket(bucketName).Object(source)
			...
			if _, err := dstObject.CopierFrom(srcObject).Run(ctx); err != nil {
				fmt.Printf("Error copying object:8 %v", err)
				continue
			}
			...
			go SizeUploadFileToGCP(client, filetrim[0], contentId, seasonId, url)
			// if errorr != nil {
			// 	fmt.Println("error in uploading size upload", errorr)
			// }
...
func SeasonFileUPloadGcp(request CreateSeasonRequestValidation, seasonId string, contentId string) {
	...
	for k := 0; k < len(newarr); k++ {
		...
		if err != nil {
			// Handle the case where the source object doesn't exist
			fmt.Printf("Source object does not exist temp: %v\n", err)
			...
			if err != nil {
				...
				// Modify the source path if needed
				filetrims := strings.Split(item, "_")
				source = contentId + "/" + seasonId + "/" + filetrims[0]
				Destination = contentId + "/" + seasonId + "/" + filetrims[0]
				srcObject = client.Bucket(bucketName).Object(source)
				attrs, err := srcObject.Attrs(ctx)
				_ = attrs
				if err != nil {
		...
		dstObject := client.Bucket(bucketName).Object(Destination)
		if _, err := dstObject.CopierFrom(srcObject).Run(ctx); err != nil {
			fmt.Printf("Error copying object:7 %v", err)
			continue
		}
...
func SeasonVarianceEpisodeImageUploadGcp(contentId string, seasonId string, EpisodeId string, nontextualImages []Images, OldcontentId string, OldseasonId string, OldEpisodeId string) {
	...
	for _, nontextrange := range nontextualImages {
		if nontextrange.HasImage {
			...
			dstObject := client.Bucket(bucketName).Object(Destination)
			if _, err := dstObject.CopierFrom(srcObject).Run(ctx); err != nil {
				fmt.Printf("Error copying object:6 %v", err)
				continue
			}
...
func SeasonVarianceFileUPloadGcp(oldseasonid string, seasonId string, contentId string) {
	...
	for k := 0; k < len(newarr); k++ {
		...
		dstObject := client.Bucket(bucketName).Object(Destination)
		if _, err := dstObject.CopierFrom(srcObject).Run(ctx); err != nil {
			fmt.Printf("Error copying object:5 %v", err)
			continue
		}
		...
		go SizeUploadFileToGCP(client, item, contentId, seasonId, url)
		// if errorr != nil {
		// 	fmt.Println("error in uploading size upload", errorr)
		// }
		// os.Remove(item)
...
func SeasonVarianceTrailerImageUploadGcp(oldcontentId string, oldseasonid string, oldTrailerId string, contentId string, seasonId string, TrailerId string, trailerPosterImage string) {
	...
	for k := 0; k < len(newarr); k++ {
		...
		if strings.Contains(item, "_") {
			...
			dstObject := client.Bucket(bucketName).Object(Destination)
			if _, err := dstObject.CopierFrom(srcObject).Run(ctx); err != nil {
				fmt.Printf("Error copying object:4 %v", err)
				continue
			}
			...
			dstObject := client.Bucket(bucketName).Object(Destination)
			if _, err := dstObject.CopierFrom(srcObject).Run(ctx); err != nil {
				fmt.Printf("Error copying object:3 %v", err)
				continue
			}
