...
func SizeUploadFileToGCP(client *storage.Client, fileName string, contentId string, seasonId string) error {
	file, err := os.Open(fileName)
	if err != nil {
		return err
	}
...
func SizeUploadFileToGcpEpi(ctx context.Context, client *storage.Client, fileName string, episodeId string, contentId string, seasonId string) error {
	file, err := os.Open(fileName)
	if err != nil {
		return err
	}
...
func SeasonTrailerImageUploadGcp(contentId string, seasonId string, TrailerId string, trailerPosterImage string) {
	...
	for k := 0; k < len(newarr); k++ {
		...
		if strings.Contains(item, "_") {
			...
			errorr := SizeUploadFileToGCP( client, filetrim[0], contentId, seasonId)
			if errorr != nil {
				fmt.Println("error in uploading size upload", errorr)
			}
...
func SeasonVarianceEpisodeImageUploadGcp(contentId string, seasonId string, EpisodeId string, nontextualImages []Images, OldcontentId string, OldseasonId string, OldEpisodeId string) {
	...
	for _, nontextrange := range nontextualImages {
		if nontextrange.HasImage {
			...
			errorr := SizeUploadFileToGcpEpi(ctx, client, nontextrange.Imagename, contentId, seasonId, EpisodeId)
			if errorr != nil {
				fmt.Println("error in uploading size upload", errorr)
			}
...
func SeasonVarianceFileUPloadGcp(oldseasonid string, seasonId string, contentId string) {
	...
	for k := 0; k < len(newarr); k++ {
		...
		errorr := SizeUploadFileToGCP( client, item, contentId, seasonId)
		if errorr != nil {
			fmt.Println("error in uploading size upload", errorr)
		}
		os.Remove(item)
