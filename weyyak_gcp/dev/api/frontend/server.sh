export SERVICE_PORT=3003 
export DB_SERVER=34.18.73.226
export DB_SERVER_READER=34.18.73.226
export DB_PORT=5432 
export DB_USER=appuser 
export DB_PASSWORD=weyyakbo345123DEV 
export DB_DATABASE=wk_frontend 
export CONTENT_DB_DATABASE=wyk_content 
export FRONTEND_CONFIG_DB_DATABASE=wyk_frontend_config 
export USER_DB_DATABASE=wk_user_management 
export DEFAULT_PAGE_SIZE=20 
export CONTENT_COMMON_API_URL= 
export CMS=https://apiqa.wyk.z5.com/v1/ 
export UM=https://apiqa.wyk.z5.com/v1/ 
export IMAGES=https://weyyak-content-dev.engro.in/ 
export GEO_LOCATION=https://geo.weyyak1.z5.com 
export VIDEO_API=https://api-weyyak.akamaized.net/get_info/ 
export AD_TAG_URL=https://s3.ap-south-1.amazonaws.com/z5xml/mobile_apps_ads_ios.xml 
export BASE_URL=https://apiqafo.engro.in/
export USER_ACTIVITY_URL=https://backofficeapi-events-dev.engro.in/event/activity 
export CONTENT_TYPE_URL=https://apiqafo.engro.in/v1/en/contents/contentType?contentType= 
export CONTENT_TYPE_URL_PAGINATION=&pageNo=1&OrderBy=desc&RowCountPerPage=50&IsPaging=0 
export USER_LOG_URL=https://backofficeapi-events-dev.engro.in/event/log 
export S3_BUCKET=z5content-uat 
export S3_URLFORCONFIG=https://weyyak-content-dev.engro.in/configqa.json 
export CONFIG_KEY=configqa 
export REDIS_CACHE_URL=https://backofficeapi-events-dev.engro.in/v1/cache
export REDIS_CONTENT_KEY=GOAPIQA 
export DOTNET_URL=https://api-backoffice-production.weyyak.com/oauth2/tokendata?access_token=
export PATH=$(go env GOPATH)/bin:$PATH
export JEAGER_URL=https://jaeger-tracer.weyyak.com/api/traces
export JEAGER_SERVICE_NAME=Frontend

export BUCKET_NAME=weyyak-dev 
export TYPE=service_account
export PROJECT_ID=engro-project-392708
export PRIVATE_KEY_ID=ee585d8aec442766ecd64561ab33c23f2ed79e71
export PRIVATE_KEY="-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"
export CLIENT_EMAIL=xmsbucket@engro-project-392708.iam.gserviceaccount.com
export CLIENT_ID="115836177213859121422"
export AUTH_URI=https://accounts.google.com/o/oauth2/auth
export TOKEN_URI=https://oauth2.googleapis.com/token
export AUTH_PROVIDER_X509_CERT_URL=https://www.googleapis.com/oauth2/v1/certs
export CLIENT_X509_CERT_URL=https://www.googleapis.com/robot/v1/metadata/x509/xmsbucket%40engro-project-392708.iam.gserviceaccount.com
export UNIVERSE_DOMAIN=googleapis.com
# swag init -g main.go
go run *.go
