...
func (hs *HandlerService) SetConfig(c *gin.Context) {
	...
	redisRequest.Key = "configqamigrate"
...
func (hs *HandlerService) GetConfigGcp(c *gin.Context) {
	var result interface{}
...
func (hs *HandlerService) SetConfigGcp(c *gin.Context) {
	...
	jsonString, _ := json.Marshal(&request)
	filename := "config2prod.json"

	// Storing to GCP Cloud Storage
	if err := common.UploadFileToGcp(jsonString, filename); err != nil {
		l.JSON(c, http.StatusInternalServerError, gin.H{"message": err})
		return
	}
