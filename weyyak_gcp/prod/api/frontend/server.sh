export SERVICE_PORT=3003 
export DB_SERVER=34.18.48.27
export DB_SERVER_READER=34.18.48.27
export DB_PORT=5432 
export DB_USER=appuser 
export DB_PASSWORD=Yy017EUY5aSz 
export DB_DATABASE=wk_frontend 
export CONTENT_DB_DATABASE=wyk_content 
export FRONTEND_CONFIG_DB_DATABASE=wyk_frontend_config 
export USER_DB_DATABASE=wk_user_management 
export DEFAULT_PAGE_SIZE=20 
export CONTENT_COMMON_API_URL= 
export CMS=https://msapifo-prod-me.weyyak.z5.com/v1/ 
export UM=https://msapifo-prod-me.weyyak.z5.com/v1/ 
export IMAGES=https://content.weyyak.com/ 
export GEO_LOCATION=https://geo.weyyak1.z5.com 
export VIDEO_API=https://api-weyyak.akamaized.net/get_info/ 
export AD_TAG_URL=https://s3.ap-south-1.amazonaws.com/z5xml/mobile_apps_ads_ios.xml 
export BASE_URL=https://msapifo-prod-me.weyyak.z5.com/
export USER_ACTIVITY_URL=https://msapiprod-me-events.z5.com/event/activity 
export CONTENT_TYPE_URL=https://msapifo-prod-me.weyyak.z5.com/v1/en/contents/contentType?contentType= 
export CONTENT_TYPE_URL_PAGINATION=&pageNo=1&OrderBy=desc&RowCountPerPage=50&IsPaging=0 
export USER_LOG_URL=https://msapiprod-me-events.com/event/log 
export S3_BUCKET=z5content-uat 
export S3_URLFORCONFIG=https://storage.googleapis.com/weyyak-production/config2prod.json 
export CONFIG_KEY=configqa 
export REDIS_CACHE_URL=https://msapiprod-me-events.z5.com/v1/cache
export REDIS_CONTENT_KEY=GOAPIPROD 
export DOTNET_URL=https://api-backoffice-production.weyyak.com/oauth2/tokendata?access_token=
export PATH=$(go env GOPATH)/bin:$PATH
export JEAGER_URL=https://jaeger-tracer.weyyak.com/api/traces
export JEAGER_SERVICE_NAME=Frontend
export BUCKET_NAME=weyyak-production 
export TYPE=service_account
export PROJECT_ID=wyk-production
export PRIVATE_KEY_ID=2d0304f0867a7a4f06d6c4369140923c82378ab2
export PRIVATE_KEY="-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"
export CLIENT_EMAIL=msapifo-prod-service-account@wyk-production.iam.gserviceaccount.com
export CLIENT_ID="117641798705593610361"
export AUTH_URI=https://accounts.google.com/o/oauth2/auth
export TOKEN_URI=https://oauth2.googleapis.com/token
export AUTH_PROVIDER_X509_CERT_URL=https://www.googleapis.com/oauth2/v1/certs
export CLIENT_X509_CERT_URL=https://www.googleapis.com/robot/v1/metadata/x509/msapifo-prod-service-account%40wyk-production.iam.gserviceaccount.com
export UNIVERSE_DOMAIN=googleapis.com
# swag init -g main.go
go run *.go
