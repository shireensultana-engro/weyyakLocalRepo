...
func (s *Server) ValidationAuthorizeRequest(r *http.Request) (*AuthorizeRequest, error) {
	...
	clientID := "000000" //TODO - Client ID to be changed   r.Form.Get("client_id")
...
func (s *Server) ValidationTokenRequest(r *http.Request) (oauth2.GrantType, *oauth2.TokenGenerateRequest, error) {
	...
	if r.FormValue("grant_type") == "apple_token" {
		gt = "apple_token"
	} else if r.FormValue("grant_type") == "facebook_token" {
		gt = "facebook_token"
	} else if r.FormValue("grant_type") == "device_code" {
		gt = "device_code"
	} else if r.FormValue("grant_type") == "twitter_token" {
		gt = "twitter_token"
	} else {
	...
	case oauth2.PasswordCredentials:
		tgr.Scope = "all" //r.FormValue("scope")
	...
	case oauth2.ClientCredentials:
		tgr.Scope = "all" //r.FormValue("scope")
	case "refresh_token": //oauth2.Refreshing:
...
func (s *Server) GetTokenData(ti oauth2.TokenInfo) map[string]interface{} {
	data := map[string]interface{}{
		...
		// "is_back_office_user": ti.GetIsBackOfficeUser(),
	}
