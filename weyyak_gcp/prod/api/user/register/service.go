...
func (hs *HandlerService) Bootstrap(r *gin.Engine) {
	// Setup Routes
	sqrg := r.Group("/v1/users")
	pbrg := r.Group("/users")
	sqrg.POST("/register_email", hs.RegisterUserUsingEmail)
	sqrg.POST("/register_sms", hs.RegisterUserUsingSMS)
	rqrgbo := r.Group("/user")
	rqrgbo.PUT("/reset_password_emails", hs.ResetPasswordEmails)
	rqrgbo.PUT("/registration_confirmation", hs.RegistrationConfirmation)
	rqrgbo.GET("/registration_confirmation", hs.RegistrationConfirmation)
	rqrgbo.DELETE("/registration_confirmation", hs.RegistrationConfirmation)
	rqrgbo.POST("/reset_password_emails", hs.ResetPasswordEmails)
	rqrgbo.GET("/reset_password_emails", hs.ResetPasswordEmails)
	rqrgbo.DELETE("/reset_password_emails", hs.ResetPasswordEmails)
	rqrg := r.Group("/v1/user")
	pqrgbo := r.Group("/user")
	pqrgbo.POST("/password", hs.ResetPasswordWithEmail)
	pqrgbo.PUT("/password", hs.ResetPasswordWithEmail)
	pqrgbo.GET("/password", hs.ResetPasswordWithEmail)
	pqrgbo.DELETE("/password", hs.ResetPasswordWithEmail)
	pqrg := r.Group("/v1/user")
	rqrg.POST("/registration_confirmation", hs.RegistrationConfirmation)
	rqrg.PUT("/reset_password_emails", hs.ResetPasswordEmails)
	pqrg.POST("/password", hs.ResetPasswordWithEmail)
	...
	dqrgbo := r.Group("/api")
	pbrbo := r.Group("/v1")
	dqrgbo.Use(common.ValidateToken())
	pbrbo.Use(common.ValidateToken())
	pbrbo.GET("/devices", hs.GetUserDevices)
	dqrgbo.GET("/devices/limit", hs.GetUserDevicesLimit)
	dqrgbo.POST("/devices/limit", hs.UpdateUserDevicesLimitCount)
	dqrg := r.Group("/v1")
	...
	pbrg.POST("/resend_email", hs.ResendEmailVerification)
	sqrg.POST("/send_otp", hs.SendOtp)
	sqrg.POST("/verify_otp", hs.VerifyOtp)
	pqrg.POST("/password_otp", hs.ForgotPasswordOtp)
	mqrg := r.Group("/v1/users")
	...
	doqrg.POST("/oauth2/logout", hs.Logout)
	...
	bqrgbo := r.Group("/api/users")
	bqrgbo.Use(common.ValidateToken())
	bqrgbo.GET("/export", hs.ExportUserDetails)
	bqrgbo.POST("/:id", hs.UpdateUserDetailsByUserid)
	bqrgbo.GET("/filters", hs.UserFilterslist)
	bqrgbo.GET("/:id/viewactivities", hs.UserViewActivitybyFilters)
	bqrgbo.GET("", hs.UsersListandSearchbyFilterswithPagination)
	bqrg := r.Group("/v1/api/users")
	...
	pqrg.PUT("/password", hs.ResetPasswordWithEmail)
	pqrg.GET("/password", hs.ResetPasswordWithEmail)
	pqrg.DELETE("/password", hs.ResetPasswordWithEmail)
	//Check paycms status
	...
	pbr := r.Group("/v1/")
	pbr.POST("/contents/playlist", hs.AddUserPlaylist)
	pbr.POST("/contents/rated", hs.AddRatingForContentByUser)
	pbr.POST("/contents/watching", hs.AddViewActivity)
...
func CreateRecordPayCMS(db *gorm.DB, user User) error {
	...
	payCMSResponse, err := common.PostCurlCall("POST", "https://zpapi-prod.weyyak.com/payment/registration/insert", PostData)
...
func (hs *HandlerService) UpdateUserProfile(c *gin.Context) {
	...
	if c.Query("platform") == "" {
		...
		if input.Aique == nil {
			input.Aique = user.Aique
		}
		if input.GoogleAnalytics == nil {
			input.GoogleAnalytics = user.GoogleAnalytics
		}
		if input.Performance == nil {
			input.Performance = user.Performance
		}
		if input.IsGdprAccepted == nil {
			input.IsGdprAccepted = user.IsGdprAccepted
		}
		if input.CleverTap == nil {
			input.CleverTap = user.CleverTap
		}
		if input.FacebookAds == nil {
			input.FacebookAds = user.FacebookAds
		}
		if input.Advertising == nil {
			input.Advertising = user.Advertising
		}
		if input.AppFlyer == nil {
			input.AppFlyer = user.AppFlyer
		}
		if input.Firebase == nil {
			input.Firebase = user.Firebase
		}
		if input.GoogleAds == nil {
			input.GoogleAds = user.GoogleAds
		}
		if err := db.Table("user").Where("id=?", userid).Update(map[string]interface{}{
			...
			"clever_tap":       input.CleverTap, "facebook_ads": input.FacebookAds, "aique": input.Aique, "advertising": input.Advertising, "apple_user_id": input.AppleUserId, "apple_sign_in": input.AppleSignIn,
		...
		if cookieChangesRow.RowsAffected == 0 {
			...
			if err := db.Table("cookie").Where("user_id=? and platform= ? ", userid, platformInt).Update(map[string]interface{}{
				...
				"apple_user_id":    inputs.AppleUserId,
				"apple_sign_in":    inputs.AppleSignIn,
				"last_activity_at": time.Now(),
			}).Error; err != nil {
				l.JSON(c, http.StatusInternalServerError, FinalResponse{"server_error", "Server error", "error_server_error", randstr.String(32)})
				return
...
func (hs *HandlerService) Logout(c *gin.Context) {
	...
	if oauthErr := db.Table("oauth2_tokens").Where("refresh=?", refreshtoken).Delete(&oauthTokens).Error; oauthErr != nil {
		// if oauthErr := db.Table("oauth2_tokens").Where("refresh=?", refreshtoken).Update("access", gorm.Expr("NULL")).Error; oauthErr != nil {
		l.JSON(c, http.StatusInternalServerError, gin.H{"error": "server_error", "description": "Server error.", "code": "error_server_error", "requestId": randstr.String(32)})
		return
	}
...
func (hs *HandlerService) GeneratePairingCode(c *gin.Context) {
	...
	if c.Request.FormValue("devicePlatform") == "" {
...
func (hs *HandlerService) SendOtp(c *gin.Context) {
	...
	fmt.Println("ffffff", userlangdetails.Id, phn.RequestType)
	...
	if phn.RequestType == "fp" {
		...
		if len(mes.Message) == 0 || mes.Phone == "" {
			...
			db.Table("otp_record").Select("*").Where("phone=(?)", phn.Phone).Find(&forcount)
...
func (hs *HandlerService) ForgotPasswordOtp(c *gin.Context) {
	...
	if userphoneresult := db.Table("user").Where("USER_NAME = ? OR NATIONAL_NUMBER = ? OR PHONE_NUMBER = ? OR USER_NAME = ? OR NATIONAL_NUMBER = ? OR PHONE_NUMBER = ? OR USER_NAME = ? OR NATIONAL_NUMBER = ? OR PHONE_NUMBER = ?", verify.PhoneNumber, verify.PhoneNumber, verify.PhoneNumber, ("+" + verify.PhoneNumber), ("+" + verify.PhoneNumber), ("+" + verify.PhoneNumber), strings.TrimLeft(verify.PhoneNumber, "0"), strings.TrimLeft(verify.PhoneNumber, "0"), strings.TrimLeft(verify.PhoneNumber, "0")).Find(&user).Error; userphoneresult != nil {
	...
	if d > 30 {
		...
		if details.Message == verify.Message && d < 30 {
			...
			if userresult := db.Table("user").Where("national_number=(?) or phone_number=(?)", verify.PhoneNumber, verify.PhoneNumber).Update(UpdatePassword{PasswordHash: hashedPassword, SaltStored: saltStored, Version: 2}).Error; userresult != nil {
				l.JSON(c, http.StatusInternalServerError, gin.H{"message": userresult.Error(), "status": http.StatusInternalServerError})
				return
			}
			l.JSON(c, http.StatusOK, gin.H{"status": 1, "message": "Your password has been changed successfully"})
...
func (hs *HandlerService) DeleteUserDevices(c *gin.Context) {
	...
	if oauthErr := db.Table("oauth2_tokens").Where("access=? and data->>'UserID' = ?", deviceToken.Token, c.MustGet("userid")).Delete(&oauthTokens).Error; oauthErr != nil {
		// if oauthErr := db.Table("oauth2_tokens").Where("access=? and data->>'UserID' = ?", deviceToken.Token, c.MustGet("userid")).Update("access", gorm.Expr("NULL")).Error; oauthErr != nil {
...
func (hs *HandlerService) AddRatingForContentByUser(c *gin.Context) {
	...
	serverError := common.ServerErrorResponses("en")
	if c.MustGet("AuthorizationRequired") == 1 {
		l.JSON(c, http.StatusUnauthorized, gin.H{"message": "Authorization has been denied for this request."})
		return
	}
...
func (hs *HandlerService) AddViewActivity(c *gin.Context) {
	...
	if request.Content.ContentType != "LiveTV" {
		...
		query := GetWatchingContentDetailsQuery(request.Content.Id)
		...
		if strings.ToLower(request.Content.ContentType) == "episode" {
			...
			query := GetWatchingContentDetailsQueryStr(episode_keys, UserId.(string))
...
func GetWatchingContentDetailsQueryStr(contentid string, userID string) string {
	query := `
		select
			va.id
		from
		...
		join view_activity va on 
			va.content_id=c.id and va.user_id = '` + userID + `' and va.playback_item_id = pi1.id
			
		where
			e.episode_key in (` + contentid + `)
	`
	return query
}
