...
func (m *Manager) GenerateAccessToken(ctx context.Context, gt oauth2.GrantType, tgr *oauth2.TokenGenerateRequest) (oauth2.TokenInfo, error) {
	...
	defer db.Close()
	cli, err := m.GetClient(ctx, tgr.ClientID)
	fmt.Println("Generate access token inside manager.....")
	type UserDetails struct {
		UserName         string `json:"username"`
		LanguageId       string `json:"languageId"`
		UserId           string `json:"userId"`
		Role             string `json:"role"`
		IsBackOfficeUser bool   `json:"is_back_office_user"`
	}
	type Details struct {
	...
	if gt == "apple_token" {
		appleToken := tgr.Request.FormValue("token")
		...
		if claims, ok := token.Claims.(jwt.MapClaims); ok {
			...
			db.Raw(`SELECT user_name,language_id,u.id as user_id,r.name as role,is_back_office_user FROM public.user u left join role r on r.id=u.role_id WHERE u.user_name=?`, claims["email"]).Scan(&user)
			db.Table("user").Where("id=?", user.UserId).Update("last_activity_at", time.Now())
			tgr.UserID = user.UserId
		} else {
			fmt.Println("hhhhhhhhhhhmanager")
			fmt.Println(err)
		}

	} else if gt == "device_code" {
		deviceCode := tgr.Request.FormValue("device_code")
		// Database
		dsn := "postgres://" + os.Getenv("DB_USER") + ":" + os.Getenv("DB_PASSWORD") + "@" + os.Getenv("DB_SERVER") + ":" + os.Getenv("DB_PORT") + "/" + os.Getenv("DB_DATABASE")
		db, _ := gorm.Open("postgres", dsn)
		defer db.Close()
		...
		var user UserDetails
		db.Raw(`SELECT user_name,language_id,u.id as user_id,r.name as role,is_back_office_user FROM public.user u left join role r on r.id=u.role_id left join pairing_code pc on pc.user_id=u.id where pc.device_code=?`, deviceCode).Scan(&user)
		tgr.UserID = user.UserId
	} else if gt == "refresh_token" {
		refreshToken := tgr.Request.FormValue("refresh_token")
		// Database
		dsn := "postgres://" + os.Getenv("DB_USER") + ":" + os.Getenv("DB_PASSWORD") + "@" + os.Getenv("DB_SERVER") + ":" + os.Getenv("DB_PORT") + "/" + os.Getenv("DB_DATABASE")
		db, _ := gorm.Open("postgres", dsn)
		defer db.Close()
		...
		var user UserDetails
		db.Debug().Raw(`select data->>'UserID' as user_id, data->>'LanguageId' as language_id  from oauth2_tokens ot where refresh = ?`, refreshToken).Scan(&user)
		db.Debug().Table("user").Where("id=?", user.UserId).Update("last_activity_at", time.Now())
		tgr.UserID = user.UserId
	} else if gt == "facebook_token" {
		facebookToken := tgr.Request.FormValue("token")
		...
		if DataResponse.Email != "" {
			db.Raw(`SELECT user_name,language_id,u.id as user_id,r.name as role,is_back_office_user FROM public.user u left join role r on r.id=u.role_id WHERE u.user_name=?`, DataResponse.Email).Find(&user)
			db.Table("user").Where("id=?", user.UserId).Update("last_activity_at", time.Now())
			tgr.UserID = user.UserId
		} else if DataResponse.Email == "" {
			db.Raw(`SELECT user_name,language_id,u.id as user_id,r.name as role,is_back_office_user FROM public.user u left join role r on r.id=u.role_id WHERE u.user_name=?`, DataResponse.ID).Find(&user)
			db.Table("user").Where("id=?", user.UserId).Update("last_activity_at", time.Now())
			tgr.UserID = user.UserId
		}
	} else if gt == "twitter_token" {
		// Database
		dsn := "postgres://" + os.Getenv("DB_USER") + ":" + os.Getenv("DB_PASSWORD") + "@" + os.Getenv("DB_SERVER") + ":" + os.Getenv("DB_PORT") + "/" + os.Getenv("DB_DATABASE")
		db, _ := gorm.Open("postgres", dsn)
		defer db.Close()
		twitterToken := tgr.Request.FormValue("token")
		twitterTokenSecret := tgr.Request.FormValue("tokensecret")
		...
		if dataResponse.Email != "" {
			db.Raw(`SELECT user_name,language_id,u.id as user_id,r.name as role FROM public.user u left join role r on r.id=u.role_id WHERE u.user_name=?`, dataResponse.Email).Find(&user)
			db.Table("user").Where("id=?", user.UserId).Update("last_activity_at", time.Now())
			tgr.UserID = user.UserId
		} else if dataResponse.Email == "" {
			db.Raw(`SELECT user_name,language_id,u.id as user_id,r.name as role FROM public.user u left join role r on r.id=u.role_id WHERE u.user_name=?`, dataResponse.ID).Find(&user)
			db.Table("user").Where("id=?", user.UserId).Update("last_activity_at", time.Now())
			tgr.UserID = user.UserId
		}
	} else {
		...
		db, _ := gorm.Open("postgres", dsn)
		defer db.Close()
		db.Raw(`SELECT user_name,language_id,u.id as user_id,r.name as role,is_back_office_user FROM public.user u left join role r on r.id=u.role_id WHERE u.id=?`, tgr.UserID).Scan(&user)
	}
	...
	ti.SetRedirectURI(tgr.RedirectURI)
	ti.SetScope(tgr.Scope)
	if tgr.Request.FormValue("DeviceId") != "" {
		ti.SetDeviceID(tgr.Request.FormValue("DeviceId"))
	} else {
		ti.SetDeviceID(tgr.Request.FormValue("deviceId"))
	}
	ti.SetGrantType(tgr.Request.FormValue("grant_type"))
	ti.SetUsername(tgr.Request.FormValue("username"))
	ti.SetPassword(tgr.Request.FormValue("password"))
	if tgr.Request.FormValue("DeviceName") != "" {
		ti.SetDeviceName(tgr.Request.FormValue("DeviceName"))
	} else {
		ti.SetDeviceName(tgr.Request.FormValue("deviceName"))
	}
	if tgr.Request.FormValue("DevicePlatform") != "" {
		ti.SetDevicePlatform(tgr.Request.FormValue("DevicePlatform"))
	} else {
		ti.SetDevicePlatform(tgr.Request.FormValue("devicePlatform"))
	}
	ti.SetLanguageId(user.LanguageId)
	ti.SetRole(user.Role)
	ti.SetIsBackOfficeUser(user.IsBackOfficeUser)
	if user.IsBackOfficeUser {
		ti.SetExpiresAt(time.Now().Add(time.Hour * time.Duration(14)))
	} else {
		ti.SetExpiresAt(time.Now().Add(time.Minute * time.Duration(30)))
	}
	//testRefreshtokenCheck
	if ti.UserID == "d432c989-e2a2-460e-95ff-15d91c8bb689" {
		ti.SetExpiresAt(time.Now().Add(3 * time.Minute).UTC())
	}
	/* Extra input parameters
	grant_type = password,username= +918919659404,password =12345678,deviceId=f936302046cc612d,deviceName=SM-A505,devicePlatform = ios
	Go to database and get all other data required data in order to generate access token userid,language id
	Update token info with all input parameter and data obtained from database, make sure that all fields are stored in database*/
	...
	if gt != "apple_token" && gt != "facebook_token" && gt != "device_code" && gt != "refresh_token" && gt != "twitter_token" {
		...
		fmt.Println("access token", tgr.UserID)
		db.Debug().Raw("SELECT data->'DeviceID' as device_id from oauth2_tokens ot WHERE access = ? ", av).Scan(&response)
		fmt.Println("DEVICE ID ", response.DeviceId)
		result := trimQuotes(response.DeviceId)
		db.Debug().Table("user_device").Where(" user_id=? AND device_id=?", tgr.UserID, result).Update("token", av)
		return ti, nil
	} else {
		av, rv, err := m.accessGenerate.Token(ctx, td, true)
		if err != nil {
			return nil, err
		}
		ti.SetAccess(av)
