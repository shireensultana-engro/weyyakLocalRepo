...
func (hs *HandlerService) Bootstrap(r *gin.Engine) {
	...
	pbr.GET("/:lang/getmenu", hs.TvHomePage)
	r.GET("/temphomepage", hs.TempHomePage)
...
func (hs *HandlerService) GetPublishingPlatforms(c *gin.Context) {
	...
	l.JSON(c, http.StatusOK, gin.H{"data": publishingplatforms})
}
...
func (hs *HandlerService) TempHomePage(c *gin.Context) {
	key := "backofficeapi-frontend-config-dev.engro.in118en3560"
	var s3response Resultdummy
	s3url := "https://content.weyyak.com/" + key + ".json"
	responses3get, err := common.GetCurlCall(s3url)
	if err != nil {
		fmt.Println(err)
		l.JSON(c, http.StatusInternalServerError, gin.H{"message": err})
		return
	}
	json.Unmarshal(responses3get, &s3response)
	l.JSON(c, http.StatusOK, s3response)
}
...
// 			l.JSON(c, http.StatusInternalServerError, gin.H{"message": menupageError})
// 		}
...
// 					l.JSON(c, http.StatusInternalServerError, gin.H{"message": contentError})
...
// 			l.JSON(c, http.StatusInternalServerError, gin.H{"message": playListError})
// 			return
// 		}
...
// 		}
// 		for _, playlists := range featuredPlaylistsResponse {
// 			fmt.Println("playlistname ", playlists.PlaylistType)
...
func (hs *HandlerService) HomePage(c *gin.Context) {
	...
	var menuPlaylsts []MenuPlaylists
	var menuPlaylst MenuPlaylists
	key := "Menus_Slider_" + pageKey + "_" + language + "_" + strconv.Itoa(country) + "_" + strconv.Itoa(platform)
	key = strings.ReplaceAll(key, "/", "_")
	key = strings.ReplaceAll(key, "?", "_")
	url := os.Getenv("REDIS_CACHE_URL") + "/" + key
	fmt.Println("Redis url -", url, "???????????")
	...
	if "" != "" {
		...
		if menupageError := db.Raw("select p.id, p.page_key,p.has_menu_poster_image,case when 'en' = lower(?) then english_page_friendly_url when 'ar' = lower(?) then arabic_page_friendly_url end as friendly_url ,	case when 'en' = lower(?) then english_meta_description when 'ar' = lower(?) then arabic_meta_description end as seo_description,case when 'en' = lower(?) then p.english_title when 'ar' = lower(?) then p.arabic_title end as title , case when page_type = 1 then 'Home' when page_type = 8 then 'Settings' when page_type = 16 then 'Favourites' when jsonb_agg(distinct ps.slider_id) = '[null]' then 'VOD' else 'Home' end as page_type from page p left join page_slider ps on ps.page_id = p.id left join slider s on s.id = ps.slider_id left join page_country pc on pc.page_id = p.id left join country c on c.id = pc.country_id where p.page_key =? and p.is_disabled = false and p.deleted_by_user_id is null group by p.id,s.slider_key ,p.page_key", language, language, language, language, language, language, pageKey).Find(&menuPage).Error; menupageError != nil {
			fmt.Println(menupageError)
			l.JSON(c, http.StatusInternalServerError, gin.H{"message": menupageError})
		}
		Result.ID = menuPage.PageKey
		...
		if pagetype.PageType == 1 {
			// Here checking for general slider if not true go get deafult slider
			res := db.Raw("select jsonb_build_object('id', slider_key, 'type', st.name, 'playlists', jsonb_agg(distinct concat(red_area_playlist_id, ',' , green_area_playlist_id, ',' ,black_area_playlist_id))) as featured from page p join page_slider ps on ps.page_id = p.id join slider s on s.id = ps.slider_id join slider_types st on st.id = s.type where p.page_key = ? and p.is_disabled = false and s.deleted_by_user_id is null and s.is_disabled = false and (s.scheduling_start_date  <= now() or s.scheduling_start_date is null ) and (s.scheduling_end_date > now() or s.scheduling_end_date is null) and p.deleted_by_user_id is null and ps.order = 1 group by s.slider_key,st.name", pageKey).Find(&FeaturedResult).RowsAffected
			if res == 0 {
				db.Raw("select jsonb_build_object('id', slider_key, 'type', st.name, 'playlists', jsonb_agg(distinct concat(red_area_playlist_id, ',' , green_area_playlist_id, ',' ,black_area_playlist_id))) as featured from page p join page_slider ps on ps.page_id = p.id join slider s on s.id = ps.slider_id join slider_types st on st.id = s.type where p.page_key = ? and p.is_disabled = false and s.deleted_by_user_id is null and s.is_disabled = false and p.deleted_by_user_id is null and ps.order = 0 group  by  s.slider_key,st.name", pageKey).Find(&FeaturedResult)
			}
		} else {
			db.Raw("select jsonb_build_object('id', slider_key, 'type', st.name, 'playlists', jsonb_agg(distinct concat(red_area_playlist_id, ',' , green_area_playlist_id, ',' ,black_area_playlist_id))) as featured from page p join page_slider ps on ps.page_id = p.id join slider s on s.id = ps.slider_id join slider_types st on st.id = s.type where p.page_key = ? and p.is_disabled = false and s.deleted_by_user_id is null and s.is_disabled = false and (s.scheduling_start_date  <= now() or s.scheduling_start_date is null ) and (s.scheduling_end_date > now() or s.scheduling_end_date is null) and p.deleted_by_user_id is null group  by s.slider_key,st.name", pageKey).Find(&FeaturedResult)
		}
		...
		if featured.ID == 0 && featured.Type == "" {
			...
			for keyv, val := range stringsnew {
				...
				/* playlist response */
				var playlistResponse FeaturedPlaylistsResponse
				db.Raw("select p.playlist_key as id,'"+ctype+"' as playlist_type,case when 'en' = lower(?) then english_title when 'ar' = lower(?) then arabic_title end as title,jsonb_agg(coalesce(case when pi2.multi_tier_content_id is not null then pi2.multi_tier_content_id end, case when pi2.one_tier_content_id is not null then pi2.one_tier_content_id end, pic.content_id) order by pi2.order)as content from playlist p left join playlist_item pi2 on pi2.playlist_id = p.id left join playlist_item_content pic on pic.playlist_item_id = pi2.id left join page_playlist pp on pp.playlist_id = p.id and pp.page_id = ? where p.id =? and p.is_disabled='false' and (p.scheduling_start_date  <= now() or p.scheduling_start_date is null ) and (p.scheduling_end_date > now() or p.scheduling_end_date is null) group by p.id,pp.order order by pp.order", language, language, menuPage.ID, val).Find(&playlistResponse)
				if playlistResponse.ID != 0 {
					featuredPlaylistsResponse = append(featuredPlaylistsResponse, playlistResponse)
				}
				if ctype == "red_playlist" {
			...
			for _, playlists := range featuredPlaylistsResponse {
				...
				if contentError := fdb.Debug().Raw("select jsonb_agg(details order by array_position(array[?], cf.content_id::text)) as cnt from content_fragment cf where cf.content_id in (?) and language in (?)  and (select cid from (select distinct unnest(translate::text[]) as cid from (select (translate(country,'[]','{}')) from content_fragment cf where cf.content_id in (?)) as ss )aa where cid= ? )= ? and (select cid from (select distinct unnest(translate::text[]) as cid from (select (translate(platform,'[]','{}')) from content_fragment cf where cf.content_id in (?)) as ss )aa where cid= ? )= ? and rights_start_date <= now() and rights_end_date >= now() and language = ? and country like '%"+res+"%' ", finalcontents, contentIds, language, contentIds, country, country, finalcontents, platform, platform, language).First(&content1).Error; contentError != nil {
					fmt.Println(contentError)
					l.JSON(c, http.StatusInternalServerError, gin.H{"message": contentError})

				}

				var contentInfo []ContentToVerify
				var finalcontentInfo []ContentToVerify

				err := json.Unmarshal([]byte(string(content1.Cnt.RawMessage)), &contentInfo)
				if err != nil {
					fmt.Println("Error:", err)
				}

				for _, contentI := range contentInfo {

					var contentStatus ContentStatus

					err := cdb.Table("content").Where("id = ?", contentI.ContentId).Find(&contentStatus)
					if err != nil {
						fmt.Println("Error:", err)
					}

					if contentStatus.ContentType == "Movie" || contentStatus.ContentType == "LiveTV" || contentStatus.ContentType == "Play" {

						var contentVarianceStatus []ContentStatus
						var cvExist bool

						// err := cdb.Table("content_variance").Where("content_id = ?", contentI.ContentId).Find(&contentVarianceStatus)
						err := cdb.Raw(`

							select * from content_variance cv 
							
							join playback_item pi2 on pi2.id = cv.playback_item_id

							join content_rights cr on cr.id = pi2.rights_id 

							where cv.content_id = ? and (cr.digital_rights_start_date <= NOW() or cr.digital_rights_start_date is null) 
						
							and (cr.digital_rights_end_date >= NOW() or cr.digital_rights_end_date is null)
						
						`, contentI.ContentId).Find(&contentVarianceStatus)
						if err != nil {
							fmt.Println("Error:", err)
						}

						for _, cv := range contentVarianceStatus {
							if cv.Status == 1 {
								cvExist = true
							}
						}

						if contentStatus.Status == 1 && cvExist {
							finalcontentInfo = append(finalcontentInfo, contentI)
						}

					} else if contentStatus.ContentType == "Series" || contentStatus.ContentType == "Program" {
						...
						join season s on
							s.content_id = c.id and s.status = 1 and s.deleted_by_user_id is null
							
						join content_rights cr on
							cr.id = s.rights_id 
							
						join episode e on
							e.season_id = s.id and e.status = 1 and e.deleted_by_user_id is null
						
						where
							c.id = ?
						
						and
							(cr.digital_rights_start_date <= NOW() or cr.digital_rights_start_date is null) 
						
						and 
							(cr.digital_rights_end_date >= NOW() or cr.digital_rights_end_date is null)
						
						group by 
							c.id,
							c.status
						`, contentI.ContentId).Find(&seasonStatus)
						if err != nil {
							fmt.Println("Error:", err)
						}

						if len(seasonStatus) > 0 {
							finalcontentInfo = append(finalcontentInfo, contentI)
						}
				...
				data, err := json.Marshal(finalcontentInfo)
				if err != nil {
					panic("failed to marshal struct")
				}

				featuredPlaylist.Content = postgres.Jsonb{RawMessage: data}
			...
			type ContentSyncInfo struct {
				ContentId string `json:"content_id"`
				Id        int    `json:"id"`
			}

			for _, featuredPlaylistId := range featuredPlaylists {

				var contentInfoRa []ContentSyncInfo

				err := json.Unmarshal([]byte(featuredPlaylistId.Content.RawMessage), &contentInfoRa)
				if err != nil {
					panic(err)
				}

				if featured.Type == "Layout A – Smart TV" {
					if featuredPlaylistId.PlaylistType == "red_playlist" {
						if len(contentInfoRa) < 7 {
							validContent = true
						}
					} else if featuredPlaylistId.PlaylistType == "green_playlist" {
						if len(contentInfoRa) < 1 {
							validContent = true
						}
					} else if featuredPlaylistId.PlaylistType == "black_playlist" {
						if len(contentInfoRa) < 1 {
							validContent = true
						}
					}
					...
					if featuredPlaylistId.PlaylistType == "black_playlist" {
						if len(contentInfoRa) < 1 {
							validContent = true
						}
					}
		...
		// Page Playlists
		var plays Plays
		db.Raw("select jsonb_agg(p4.id order by pp2.order)::varchar as playlists from page p3 join page_playlist pp2 on pp2.page_id = p3.id join playlist p4 on p4.id = pp2.playlist_id where p4.deleted_by_user_id is null and p4.is_disabled = false and p3.page_key = ?", pageKey).Find(&plays)
		var parts []string
		if err := json.Unmarshal([]byte(plays.Playlists), &parts); err != nil {
			fmt.Println(err)
		}
		...
		if playListError := db.Raw("select p.playlist_key as id,case when 'en'=lower('"+language+"') then p.english_title when 'ar'=lower('"+language+"') then p.arabic_title end as playlist_type,jsonb_agg(coalesce(case when pi2.multi_tier_content_id is not null then pi2.multi_tier_content_id end, case when pi2.one_tier_content_id is not null then pi2.one_tier_content_id end,case when pi2.season_id is not null then pi2.season_id end,pic.content_id) order by pi2.order)as content from playlist p join playlist_item pi2 on pi2.playlist_id = p.id join playlist_item_content pic on pic.playlist_item_id = pi2.id join page_playlist pp on pp.playlist_id = p.id and pp.page_id =? where p.id in (?) and p.is_disabled='false' and (p.scheduling_start_date  <= now() or p.scheduling_start_date is null ) and (p.scheduling_end_date > now() or p.scheduling_end_date is null) group by p.id,pp.order order by pp.order", menuPage.ID, playlistIds).Find(&featuredPlaylistsResponse).Error; playListError != nil {
			fmt.Println(playListError)
			l.JSON(c, http.StatusInternalServerError, gin.H{"message": playListError})
			return
		}
		...
		for _, playlists := range featuredPlaylistsResponse {
			...
			if len(finalcontentids) > 0 {
				var content2 Cont
				res1 := strconv.Itoa(country)
				if contentError := fdb.Raw("select jsonb_agg(details order by array_position(array[?], cf.content_id::text)) as cnt from (SELECT DISTINCT ON (content_key, language) * FROM content_fragment cf where (cf.content_id in (?) or  cf.content_variance_id in (?))and language in (?)  and (select cid from (select distinct unnest(translate::text[]) as cid from (select (translate(country,'[]','{}')) from content_fragment cf where (cf.content_id in (?) or  cf.content_variance_id in (?))) as ss )aa where cid= ? )= ? and (select cid from (select distinct unnest(translate::text[]) as cid from (select (translate(platform,'[]','{}')) from content_fragment cf where (cf.content_id in (?) or cf.content_variance_id in (?))) as ss )aa where cid= ? )= ? and rights_start_date <= now() and rights_end_date >= now() and language = ? and country like '%"+res1+"%' ) cf;", contentidsfororder, finalcontentids, finalcontentids, language, finalcontentids, finalcontentids, country, country, finalcontentids, finalcontentids, platform, platform, language).Find(&content2).Error; contentError != nil {
					fmt.Println(contentError)
					l.JSON(c, http.StatusInternalServerError, gin.H{"message": contentError})
					return
				}

				var contentInfo2 []ContentToVerify
				var finalcontentInfo2 []ContentToVerify

				err := json.Unmarshal([]byte(string(content2.Cnt.RawMessage)), &contentInfo2)
				if err != nil {
					fmt.Println("Error:", err)
				}

				for _, contentI := range contentInfo2 {
					...
					if contentStatus.ContentType == "Movie" || contentStatus.ContentType == "LiveTV" || contentStatus.ContentType == "Play" {
						...
						if contentStatus.Status == 1 && cvExist2 {
							finalcontentInfo2 = append(finalcontentInfo2, contentI)
						}

					} else if contentStatus.ContentType == "Series" || contentStatus.ContentType == "Program" {

						var seasonStatus2 []SeasonStatus
						...
						err := cdb.Raw(`
							...
							group by 
								c.id,
								c.status
						`, contentI.ContentId).Find(&seasonStatus2)
						if err != nil {
							fmt.Println("Error:", err)
						}

						if len(seasonStatus2) > 0 {
							finalcontentInfo2 = append(finalcontentInfo2, contentI)
						}
				...
				data2, err := json.Marshal(finalcontentInfo2)
				if err != nil {
					panic("failed to marshal struct")
				}

				menuPlaylst.Content = postgres.Jsonb{RawMessage: data2}
				// menuPlaylst.Content = content2.Cnt
				menuPlaylsts = append(menuPlaylsts, menuPlaylst)
			} else {
				continue
...
func (hs *HandlerService) UpdateAllPageCache(c *gin.Context) {
	...
	if len(pageKeys) > 0 {
		for _, keys := range pageKeys {
			...
			search := "backofficeapi-frontend-dev.engro.in_v1*"
			fmt.Println(search, "??????????????")
			if len(os.Args) > 1 {
				search = os.Args[1]
			}
			...
			if (counts.Slidercount > 0 || counts.Playlistcount > 0) || counts.IsDisabled == 0 {
				...
				l.JSON(c, http.StatusOK, "job success")
			} else {
				l.JSON(c, http.StatusOK, "The page (psge_key "+strconv.Itoa(keys.PageKey)+") has no sliders or playlists. so, its not mandatory to prepare cache for this page.")
				// return
			}
		}
...
// 			c, err := net.Dial("tcp", "10.0.0.18:6379")
// 			if err != nil {
// 				log.Fatal(err)
// 				return nil, err
// 			}
...
func (hs *HandlerService) TvHomePage(c *gin.Context) {
	...
	key := "Menus_Slider_" + language + "_" + country + "_" + device
	...
	if RedisResponse.Value != "" {
		...
		url := os.Getenv("BASE_URL") + "v1/" + language + "/menu?device=" + device + "&country=" + country
		method := "GET"
		client := &http.Client{}
		...
		for _, singlePage := range menuPage.Data {
			...
			url := os.Getenv("BASE_URL") + "v1/" + language + "/menu/" + strconv.Itoa(singlePage.ID) + "?cascade=2&country=" + country + "&platform=" + device
			method := "GET"
			fmt.Println(url)
			client := &http.Client{}
			req, err := http.NewRequest(method, url, nil)
			...
			if data.Data.ID != 67 && data.Data.ID != 89 && data.Data.ID != 74 {
	...
	l.JSON(c, http.StatusOK, menuPage)
	return
}
