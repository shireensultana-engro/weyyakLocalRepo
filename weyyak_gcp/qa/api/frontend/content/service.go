		...
		if seasonDetails != nil {
			for i, details := range seasonDetails {
				...
				if seasonLimit == 0 {
					...
					var seasonEpisodes []SeasonEpisodesPlans
					var episodesResponse []SeasonEpisodesPlans
					...
					if err := db.Debug().Select(fields).Table("episode e").
						...
						for _, episode := range seasonEpisodes {
							var SubsPlansEpi []ContentSubsPlans
							// fields, join, where := common.ContentPlansQuery(2)
							fmt.Println("***inside go routine MultiTierContentDetails ContentPlansQuery query")
						...
						where
							...
							for _, plan := range SubsPlansEpi {
								...
								var planNameSubsEpi SubscriptionPlan
								if err := db.Debug().Table("subscription_plan").Where("id = ?", plan.SubscriptionPlanId).Find(&planNameSubsEpi).Error; err != nil {
									return
								}
							...
							episode.SubscriptionPlans = EpiPlans
							episode.SubscriptionPlansName = EpiPlansName
	...
	if row := db.Table("content c").Select(fields).Joins(join).Where(where, contentId, country).Group(groupBy).Order("s.number").Limit(1).Find(&onetierContentResult).RowsAffected; row != 0 {
		...
		if seasonDetails != nil {
			for i, details := range seasonDetails {
				...
				// var episodesResponse []SeasonEpisodes
				season.Episodes = make([]SeasonEpisodesPlans, 0)
				seasons = append(seasons, season)
			}
		}
		playlistContent.Seasons = seasons
...
func OnetierImagery(contentId, varianceId string, ImageryDetails chan ContentImageryDetails) {
	...
	Imagery.Banner = os.Getenv("IMAGES") + contentId + "/" + varianceId + "/overlay-poster-image"
	ImageryDetails <- Imagery
}
func MultitierImagery(contentId, seasonId string, ImageryDetails chan ContentImageryDetails) {
	var Imagery ContentImageryDetails
	Imagery.Thumbnail = os.Getenv("IMAGES") + contentId + "/" + seasonId + "/poster-image"
	Imagery.Backdrop = os.Getenv("IMAGES") + contentId + "/" + seasonId + "/details-background"
	Imagery.MobileImg = os.Getenv("IMAGES") + contentId + "/" + seasonId + "/mobile-details-background"
	Imagery.FeaturedImg = os.Getenv("IMAGES") + contentId + "/" + seasonId + "/poster-image"
	Imagery.Banner = os.Getenv("IMAGES") + contentId + "/" + seasonId + "/overlay-poster-image"
	ImageryDetails <- Imagery
}
func EpisodeImagery(contentId, seasonId, episodeId string, ImageryDetails chan ContentImageryDetails) {
	var Imagery ContentImageryDetails
	Imagery.Thumbnail = os.Getenv("IMAGES") + contentId + "/" + seasonId + "/" + episodeId + "/poster-image"
	Imagery.Backdrop = os.Getenv("IMAGES") + contentId + "/" + seasonId + "/details-background"
	Imagery.MobileImg = os.Getenv("IMAGES") + contentId + "/" + seasonId + "/mobile-details-background"
	Imagery.FeaturedImg = os.Getenv("IMAGES") + contentId + "/" + seasonId + "/" + episodeId + "/poster-image"
	Imagery.Banner = os.Getenv("IMAGES") + contentId + "/" + seasonId + "/overlay-poster-image"
	ImageryDetails <- Imagery
}
