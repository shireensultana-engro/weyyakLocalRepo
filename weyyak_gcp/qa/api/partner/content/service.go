...
func (hs *HandlerService) Bootstrap(r *gin.Engine) {
	...
	srg.GET("/contents/deleted", hs.GetDeletedContentDetails)
	srg.GET("/contents/modified", hs.GetModifiedContentDetails)
}
...
func (hs *HandlerService) GetOneTierContentDetailsBasedonContentID(c *gin.Context) {
	...
	var finalContentResult []FinalSeasonResultOneTier
...
// GetDeletedContentDetails- Get All Deleted Content Details
// GET /contents/deleted
// @Description Get All Deleted Content Details
// @Tags Content Updates
// @Accept  json
// @Security Authorization
// @Produce  json
// @Param from query string false "Date from which deleted data to be fetched." format(date) example(2000-01-01)
// @Param to query string false "Date to which deleted data to be fetched." format(date) example(2000-01-01)
// @Success 200  object map[string]interface{}
// @Failure 404 "The object was not found."
// @Failure 500 object ErrorResponse "Internal server error."
// @Router /v1/contents/deleted [get]
func (hs *HandlerService) GetDeletedContentDetails(c *gin.Context) {
	fmt.Println("*gyhbhbhjbhb")
	if c.MustGet("AuthorizationRequired") == 1 {
		c.JSON(http.StatusUnauthorized, gin.H{"message": "Authorization has been denied for this request."})
		return
	}
	...
	fmt.Println("user idddddddd", UserId)

	db := c.MustGet("DB").(*gorm.DB)

	fmt.Println("user iddd1111111111111", UserId)
	...
	if startdateStr == "" && enddateStr == "" {
		...
		startdate, err = time.Parse("2006-01-02", enddateStr)
		if err != nil {
	...
	if days > 7 {
		...
		// 	c.JSON(http.StatusBadRequest, gin.H{"message": "Please provide start date less than or equal to today's date"})
		// 	return
...
// GetModifiedContentDetails- Get All Modified Content Details
// GET /contents/modified
// @Description Get All Modified Content Details
// @Tags Content Updates
// @Accept  json
// @Security Authorization
// @Produce  json
// @Param from query string false "Date from which modified data to be fetched." format(date) example(2000-01-01)
// @Param to query string false "Date to which modified data to be fetched." format(date) example(2000-01-01)
// @Success 200  object map[string]interface{}
// @Failure 404 "The object was not found."
// @Failure 500 object ErrorResponse "Internal server error."
// @Router /v1/contents/modified [get]
func (hs *HandlerService) GetModifiedContentDetails(c *gin.Context) {
	...
	if days > 7 {
		...
		var finalOnetierContentResult []FinalSeasonResultOneTier
		var totalCount int

		var contentResultFinal []MultiTierContent2
		var finalContent []FinalSeasonResultContentOneTier
		var totalMultiCount []FinalSeasonResultContentOneTier
