...
func (hs *HandlerService) CreateProgamList(c *gin.Context) {
	...
	if err := db.Debug().Create(&Channel).Error; err != nil {
		...
		go programLogoUploadGcp(createChannelProgramList.Logo, createChannelProgramList.Channel, createChannelProgramList.Name)
		l.JSON(c, http.StatusOK, gin.H{"message": "program sheduled Successfully."})
		return
	}
}
...
func (hs *HandlerService) GetProgamList(c *gin.Context) {
	...
	l.JSON(c, http.StatusOK, gin.H{"data": channelProgramList})
}
...
func (hs *HandlerService) GetProgramlistByChannelName(c *gin.Context) {
	...
	l.JSON(c, http.StatusOK, channelProgramList)
}
