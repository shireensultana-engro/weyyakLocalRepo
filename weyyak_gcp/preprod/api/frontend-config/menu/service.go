...
func (hs *HandlerService) Bootstrap(r *gin.Engine) {
	...
	qrg1.GET("/:lang/getmenu", hs.TvHomePage)
...
// 					contentidsfororder = append(contentidsfororder, val)
// 				}
// 				cdb.Debug().Raw("select count(id) from content where (content_type = 'Series' or content_type = 'Program') and id = ? ", val).Count(&mltcount)
// 				cdb.Debug().Raw("select count(id) from content where (content_type = 'Movie' or content_type = 'LiveTV' or content_type  = 'Play') and id = ?", val).Count(&moviecount)
// 				if mltcount > 0 {
...
func (hs *HandlerService) UpdateAllPageCache(c *gin.Context) {
	...
	if len(pageKeys) > 0 {
		for _, keys := range pageKeys {
			...
			search := "msapiprod-me-frontend.z5.com_v1*"
			fmt.Println(search, "??????????????")
			if len(os.Args) > 1 {
				search = os.Args[1]
			}
...
func (hs *HandlerService) TvHomePage(c *gin.Context) {
	...
	if RedisResponse.Value != "" {
		...
		url := "https://msapifo-prod-me.weyyak.z5.com/v1/" + language + "/menu?device=" + device
		method := "GET"
		client := &http.Client{}
		...
		for _, singlePage := range menuPage.Data {
			...
			url := "https://msapifo-prod-me.weyyak.z5.com/v1/" + language + "/menu/" + strconv.Itoa(singlePage.ID) + "?cascade=2&country=" + country
			method := "GET"
			fmt.Println(url)
			client := &http.Client{}
			req, err := http.NewRequest(method, url, nil)
