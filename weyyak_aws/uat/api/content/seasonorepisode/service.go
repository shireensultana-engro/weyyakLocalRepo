...
func (hs *HandlerService) DeleteSeason(c *gin.Context) {
	...
	if count > 0 {
		...
		//TODO: Testing season delete
		// fragments.RemoveContentFragmentSeason(c, c.Param("id"))
		// in previous code there is not wait group for the concurrency, so testing purpose im changing this funtion to direct code
		var deleteSeason fragments.ContentFragment
		if err := fdb.Where("content_variance_id = ?", c.Param("id")).Delete(&deleteSeason).Error; err != nil {
			log.Println("Error deleting fragment:", err)
		} else {
			log.Println("Fragment deleted successfully")
		}
		//TODO: Testing season delete
