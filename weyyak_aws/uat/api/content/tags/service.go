...
func (hs *HandlerService) CreateTags(c *gin.Context) {
	...
	if Tags.Name == "" {
		...
		c.JSON(http.StatusBadRequest, gin.H{"error": "invalid_request", "description": "Validation failed.", "code": "error_validation_failed", "requestId": randstr.String(32), "name": name})
		return
	}
	if err := db.Debug().Where("name=?", Tags.Name).Find(&Tags).RowsAffected; err != 0 {
		...
		if err := db.Debug().Create(&Tags).Error; err != nil {
			...
			c.JSON(http.StatusOK, gin.H{"message": "Tags Created Successfully.", "Status": http.StatusOK})
			return
		}
	}
}
