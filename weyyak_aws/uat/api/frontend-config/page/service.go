...
func (hs *HandlerService) CreateOrUpdatePageDetails(c *gin.Context) {
	...
	if c.Param("id") == "" {
		...
		fmt.Println("pageres---------->",pageres)
		
		if pageresult := tx.Debug().Table("page").Create(&pageres).Error; pageresult != nil {
			tx.Rollback()
			fmt.Printf("Create or update page %s : Error in creating page query %s", pageRequest.EnglishTitle, pageresult)
			c.JSON(http.StatusInternalServerError, errorresponse)
			return
		}
		...
		if pageplaylistres := gormbulk.BulkInsert(tx, pageplayarr, common.BULK_INSERT_LIMIT); pageplaylistres != nil {
			tx.Rollback()
			fmt.Printf("Create or update page %s : Error in page playlists %s", pageRequest.EnglishTitle, pageplaylistres)
			c.JSON(http.StatusInternalServerError, errorresponse)
			return
		}
