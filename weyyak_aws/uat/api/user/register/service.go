...
func (hs *HandlerService) SendOtp(c *gin.Context) {
	...
	if phn.RequestType == "fp" {
		data := db.Table("otp_record").Select("message,phone").Where("phone=?", phonenumberfinal)
		data.Scan(&mes)
		if len(mes.Message) == 0 || mes.Phone == "" {
			user := UserDetails{Phone: phonenumberfinal, Message: otp, SentOn: time.Now(), Number: 1}
			if final := db.Table("otp_record").Create(&user).Error; final != nil {
				c.JSON(http.StatusInternalServerError, gin.H{"message": final.Error(), "status": http.StatusInternalServerError})
				return
			}
		} else {
			db.Table("otp_record").Select("number").Where("phone=(?)", phonenumberfinal).Find(&forcount)
			if forcount.Number <= 6 {
				if result := db.Table("otp_record").Where("phone=(?)", phonenumberfinal).Update(UserDetails{Message: otp, SentOn: time.Now(), Number: forcount.Number + 1}).Error; result != nil {
					c.JSON(http.StatusInternalServerError, gin.H{"message": result.Error(), "status": http.StatusInternalServerError})
					return
				}
			}
	...
	if forcount.Number < 6 {
		if phn.RequestType == "up" {
			...
			if sample != nil {
				c.JSON(http.StatusInternalServerError, gin.H{"message": sample.Error(), "status": http.StatusInternalServerError})
				return
			}
		} else {
			_, sample := svc.Publish(params)
		...
		if phn.RequestType == "fp" {
			db.Table("otp_record").Select("sent_on").Where("phone=(?)", phonenumberfinal).Find(&senttime)
		} else {
		...
		if lastSent < currentTime {
			...
			if result := db.Table("otp_record").Where("phone=(?)", phn.Phone).Update(UserDetails{Message: otp, SentOn: time.Now(), Number: 1}).Error; result != nil {
				c.JSON(http.StatusInternalServerError, gin.H{"message": result.Error(), "status": http.StatusInternalServerError})
				return
			}
		} else {
...
// ExportUserDetails -  Export user details
...
func (hs *HandlerService) ExportUserDetails(c *gin.Context) {
	...
	db.Raw(`select
				...
				con.english_name as country, us.registered_at, us.email, us.phone_number,    
	...
	if fileType == "csv" {
		...
		for _, row := range final {
			_ = csvwriter.Write([]string{
				...
				row.Status,
				row.Country,
				row.RegisteredAt.String(),
				row.Email,
				row.PhoneNumber,
				row.TailoredGenres,
				row.ActiveDevices,
				strconv.Itoa(row.NumberOfActiveDevices),
				row.Language,
		...
		for i := 0; i < len(final); i++ {
			...
			f.SetCellValue("Sheet1", "I"+strconv.Itoa(i+2), final[i].ActiveDevices)
		...
		c.JSON(400, gin.H{"status": 0, "message": "Worng File Type"})
	}
