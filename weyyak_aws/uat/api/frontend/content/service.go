...
func (hs *HandlerService) GetSearchbyGenre(c *gin.Context) {
	...
	if len(queries) > 2 {
		...
		querystring = strings.ToLower(querystring + "&" + params[len(params)-1:][0])
	} else {
	...
	querydata := "select distinct on (c.content_key)c.content_key, g.id as genre_id, c.id as content_id, c.content_tier,  c.created_at as inserted_at, c.modified_at, lower(c.content_type) as content_type, cpi.transliterated_title as title  " + language_translated_title + " , cpi.transliterated_title as friendly_url,  min(pi1.video_content_id) as video_id ,cv.id as season_or_varience_id  from genre g  join content_genre cg on g.id = cg.genre_id join content c  on cg.content_id = c.id  join content_primary_info cpi on c.primary_info_id = cpi.id  join content_variance cv on cv.content_id =c.id  join playback_item pi1 on pi1.id =cv.playback_item_id  join content_rights cr on cr.id=pi1.rights_id  join content_translation ct on ct.id=pi1.translation_id  full outer join content_rights_country crc on crc.content_rights_id =cr.id  full outer join content_rights_plan crp on crp.rights_id =cr.id where  c.deleted_by_user_id is null  and c.status =1  and c.deleted_by_user_id is null  and ( pi1.scheduling_date_time  <= NOW() or pi1.scheduling_date_time is null)   and (cr.digital_rights_start_date <=NOW() or cr.digital_rights_start_date is null)  and (cr.digital_rights_end_date >=NOW() or cr.digital_rights_end_date is null)"

	if querystring != "" {
		querydata += "  and crc.country_id='" + country + "'  and (  lower(g.english_name) ='" + querystring + "'  or lower(g.arabic_name) = '" + querystring + "'  )"
	}
	querydata += "  group by g.id,c.content_key,c.id,cpi.transliterated_title,cpi.arabic_title,cv.id union  select distinct on (c.content_key)c.content_key,g.id as genre_id, c.id as content_id, c.content_tier,  c.created_at as inserted_at, c.modified_at, lower(c.content_type) as content_type, cpi.transliterated_title as title   " + language_translated_title + " ,  cpi.transliterated_title as friendly_url,  min(pi1.video_content_id) as video_id ,s.id as season_or_varience_id from genre g  join content_genre cg on g.id = cg.genre_id join content c  on cg.content_id = c.id  join content_primary_info cpi on c.primary_info_id = cpi.id  join season s on c.id = s.content_id left join episode e on s.id = e.season_id  join playback_item pi1 on e.playback_item_id =pi1.id  join content_rights cr on cr.id=pi1.rights_id  join content_translation ct on ct.id=pi1.translation_id  full outer join content_rights_country crc on crc.content_rights_id =cr.id  full outer join content_rights_plan crp on crp.rights_id =cr.id where  c.deleted_by_user_id is null   and c.status =1  and s.status=1  and c.deleted_by_user_id is null  and ( pi1.scheduling_date_time  <= NOW() or pi1.scheduling_date_time is null)   and (cr.digital_rights_start_date <=NOW() or cr.digital_rights_start_date is null)  and (cr.digital_rights_end_date >=NOW() or cr.digital_rights_end_date is null)  and crc.country_id='" + country + "'  "

	if querystring != "" {
		querydata += "  and (  lower(g.english_name)  ='" + querystring + "'  or lower(g.arabic_name)  = '" + querystring + "'  )  "
	}
	querydata += " group by g.id,c.content_key,c.id,cpi.transliterated_title,cpi.arabic_title ,s.id order by inserted_at desc"

	if err := db.Raw(querydata).Limit(100).Find(&finalResult).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "server_error", "description": "Server error.", "code": "error_server_error", "requestId": randstr.String(32)})
		return
	}
	...
	if len(contents) == 0 {
		c.JSON(http.StatusOK, gin.H{"data": emptyData})
		return
	}
	c.JSON(http.StatusOK, gin.H{"data": contents})
	return
...
func (hs *HandlerService) GetPlaylistDetails(c *gin.Context) {
	...
	fmt.Println("0-------------0")
	fmt.Println("0-------------0", playlist.PlaylistType)
