	...
	if "" != "" {
		...
		for _, playlist := range playlists {
			...
			if playlist.PlaylistType == "pagecontent" {
				...
				if err := fdb.Table("content").Debug().Select("details::text as details").Where("content_id in(?) and country=? and language=? and platform=?", Ids, countryCode, language, platformName).Find(&contentFragmentDetails).Error; err != nil {
					c.JSON(http.StatusInternalServerError, serverError)
					return
				}
				...
				for _, content := range contentFragmentDetails {
					...
					type ContentDetailsForCheck struct {
						Status int `json:"status"`
					}

					var contentDetailsForCheck ContentDetailsForCheck
					if err := cdb.Table("content").Where("content_id = ?", playlistContent.ContentId).Find(&contentDetailsForCheck).Error; err != nil {
						return
					}

					if playlistContent.ID != 0 && contentDetailsForCheck.Status == 1 {
						playlistContents = append(playlistContents, playlistContent)
					}
				}
