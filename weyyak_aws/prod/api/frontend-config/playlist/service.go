...
func (hs *HandlerService) GetPlaylistItemBasedOnPlaylistItemTypeAndItemTypeID(c *gin.Context) {
	...
	case 5:
		...
		break
	case 6:
		Query := "select cpi.transliterated_title as name,c.content_tier as type,c.id from content c join content_cast cc on cc.id=c.cast_id join actor a on a.id =cc.main_actor_id or a.id=cc.main_actress_id join content_primary_info cpi on cpi.id =c.primary_info_id where a.id='8d2b9172-464b-e911-8259-02e5c03f648e' and c.deleted_by_user_id is null union select CONCAT(cpi2.transliterated_title,' season ',',№',s.Number)as name,'3',s.id from season s join content_cast cc2 on cc2.id =s.cast_id join actor a2 on a2.id =cc2.main_actor_id or a2.id=cc2.main_actress_id join content_primary_info cpi2 on cpi2.id=s.primary_info_id where a2.id='8d2b9172-464b-e911-8259-02e5c03f648e' and s.deleted_by_user_id is null"
		rows := cdb.Debug().Raw(Query).Find(&playListItem)
		totalcount = int(rows.RowsAffected)
		if actorResult := cdb.Raw(Query).Limit(limit).Offset(offset).Scan(&playListItem).Error; actorResult != nil {
			c.JSON(http.StatusInternalServerError, errorresponse)
...
func (hs *HandlerService) CreateAndUpdatePlaylist(c *gin.Context) {
	...
	if playlistid != "" {
		...
		if err := fcdb.Debug().Table("playlist").Select("max(playlist_key)+1 as key").Find(&playlistKey).Error; err != nil {
			c.JSON(http.StatusInternalServerError, errorresponse)
			return
		}
