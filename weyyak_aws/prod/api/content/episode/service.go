...
func (hs *HandlerService) CreateOrUpdateEpisodes(c *gin.Context) {
	...
	if c.Param("id") == "" {
		...
		go EpisodeFileUPload(episodes, createEpisode.Id, seasonDetails.ContentId, episodes.SeasonId)
		...
		/* upload images to S3 bucket based on episode Id*/
		go EpisodeFileUPload(episodes, c.Param("id"), seasonDetails.ContentId, episodes.SeasonId)
