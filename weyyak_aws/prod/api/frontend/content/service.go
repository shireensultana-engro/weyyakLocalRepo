...
func (hs *HandlerService) Bootstrap(r *gin.Engine) {
	...
	qrg.GET("/:lang/series", hs.GetSeasonsByContent)
...
func (hs *HandlerService) GetContentsByPlanId(c *gin.Context) {
	...
	if Fragment == "" {
		...
		for _, spcd := range spcds {
			if spcd.ContentTier == 1 {
				...
				go MultiTierContentDetails(spcd.Id, language, country, c, ContentDetails, 1)
			}
	...
	c.JSON(http.StatusOK, gin.H{"pagination": pagination, "data": contents})
	return
}
...
func OneTierContentDetails(contentId string, language string, country int, c *gin.Context, ContentDetails chan PlaylistContent) {
	...
	if row := db.Table("content c").Select(fields).Joins(join).Where(where, contentId, country).Group(groupBy).Find(&onetierContentResult).RowsAffected; row != 0 {
		...
		*/
		...
		if err := db.Table("content c").Select(movieFields).Joins(join).Where(where, contentId, country).Group(groupBy).Order("cv.order").Find(&movieDetails).Error; err != nil {
			fmt.Println("err", err)
			return
		}
...
func MultiTierContentDetails(contentId string, language string, country int, c *gin.Context, ContentDetails chan PlaylistContent, seasonLimit int) {
	...
	if row := db.Table("content c").Select(fields).Joins(join).Where(where, contentId, country).Group(groupBy).Order("e.number,s.number").Limit(1).Find(&onetierContentResult).RowsAffected; row != 0 {
		playlistContent.ID = onetierContentResult.ID
		playlistContent.AgeRating = common.AgeRatings(onetierContentResult.AgeRating, language)
		playlistContent.VideoId = onetierContentResult.VideoId
		friendlyUrl := strings.ToLower(onetierContentResult.FriendlyUrl)
	...
	if RedisResponse.Value != "" {
		...
		if details.ID == 0 {
			...
			if details1.ID == 0 && details.ID == 0 {
				...
				c.JSON(http.StatusOK, gin.H{"data": details1})
				return
			}
		}
		/* Create Redis Key for content Type */
...
func (hs *HandlerService) GetContentRating(c *gin.Context) {
	...
	if c.Param("ckeyctype") == "" {
		c.JSON(http.StatusBadRequest, gin.H{"message": "Please Provide Content Key and Content Type.", "status": http.StatusBadRequest})
		return
	}
	request := strings.Split(c.Param("ckeyctype"), ",")
	var ContentKey string
	if len(request) < 2 {
		c.JSON(http.StatusBadRequest, gin.H{"message": "Please Provide valid Content Key and Content Type.", "status": http.StatusBadRequest})
		return
	} else {
...
func (hs *HandlerService) GetContentTypeDetails(c *gin.Context) {
	...
	if c.Request.URL.Query()["contentType"] == nil || c.Request.URL.Query()["contentType"][0] == "" {
		c.JSON(http.StatusBadRequest, gin.H{"message": "Please provide content type."})
		return
	}
	...
	if RedisResponse.Value != "" {
		...
		if contentType == "series" || contentType == "program" {
			...
			if IsPaging == 1 {
				...
				if err := db.Raw("select c.id as content_id,min(s.id::text) as season_id,c.content_key as id,c.content_tier,min(pi1.video_content_id) as video_id,replace(lower(cpi.transliterated_title), ' ', '_') as friendly_url,cpi.transliterated_title as title,c.created_at as time from content_primary_info cpi join content c on c.primary_info_id = cpi.id join season s on s.content_id = c.id join episode e on e.season_id = s.id join playback_item pi1 on pi1.id = e.playback_item_id join content_rights cr on cr.id = s.rights_id join content_rights_country crc on crc.content_rights_id = cr.id where(( pi1.scheduling_date_time <= NOW() or pi1.scheduling_date_time is null) and (cr.digital_rights_start_date <= NOW() or cr.digital_rights_start_date is null) and (cr.digital_rights_end_date >= NOW() or cr.digital_rights_end_date is null) and c.status = 1 and c.deleted_by_user_id is null and s.status = 1 and s.deleted_by_user_id is null and e.status = 1 and e.deleted_by_user_id is null and crc.country_id = ? and lower(c.content_type) = ?) group by c.id,c.content_key,cpi.transliterated_title,cpi.arabic_title,c.created_at union select c.id as content_id,min(s.id::text) as season_id,c.content_key as id,c.content_tier,min(vt.video_trailer_id) as video_id,replace(lower(cpi.transliterated_title), ' ', '_') as friendly_url,cpi.transliterated_title as title,c.created_at as time from content_primary_info cpi left join content c on c.primary_info_id = cpi.id left join season s on s.content_id = c.id left join episode e on e.season_id = s.id  left join variance_trailer vt on vt.season_id = s.id left join content_rights cr on cr.id = s.rights_id left join content_rights_country crc on crc.content_rights_id = cr.id where((cr.digital_rights_start_date <= NOW() or cr.digital_rights_start_date is null)and (cr.digital_rights_end_date >= NOW() or cr.digital_rights_end_date is null)and c.status = 1 and c.deleted_by_user_id is null and s.status = 1 and s.deleted_by_user_id is null  and vt.id is not null and crc.country_id = ? and lower(c.content_type) = ? and e.id is null) group by c.id,c.content_key,cpi.transliterated_title,cpi.arabic_title,c.created_at order by time "+OrderBy+" ", country, contentType, country, contentType).Find(&contents).Error; err != nil {
					c.JSON(http.StatusInternalServerError, gin.H{"error": err})
					return
				}
			}
			...
			if IsPaging == 1 {
				if err := db.Select(fields).Table("content c").Joins(join).Where(where, country, contentType).Group(groupBy).Order("c.created_at  " + OrderBy + "").Limit(limit).Offset(offset).Find(&contents).Error; err != nil {
					c.JSON(http.StatusInternalServerError, gin.H{"error": err})
					return
				}
			} else {
				if err := db.Select(fields).Table("content c").Joins(join).Where(where, country, contentType).Group(groupBy).Order("c.created_at  " + OrderBy + "").Find(&contents).Error; err != nil {
					c.JSON(http.StatusInternalServerError, gin.H{"error": err})
					return
				}
			}
...
func (hs *HandlerService) GetContentTrailerDetails(c *gin.Context) {
	...
	for _, trailer := range trailers {
		...
		imagery.TrailerPosterImage = os.Getenv("IMAGES") + trailer.ContentId + "/" + trailer.VarianceId + "/" + trailer.Id + "/trailer-poster-image"
...
func (hs *HandlerService) GetMediaObjectDetails(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"data": mediaObjects})
	return
}
...
func (hs *HandlerService) GetUserPlaylists(c *gin.Context) {
	...
	if err := db.Table("content c").Select("c.content_key::text").Joins("join playlisted_content pc on pc.content_id =c.id").Where("pc.user_id=?", UserId).Order("pc.added_at desc").Count(&totalCount).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "server_error", "description": "حدث خطأ ما", "code": "error_server_error", "requestId": UserId.(string)})
		return
	}
	...
	c.JSON(http.StatusOK, gin.H{"pagination": pagination, "data": response})
	return
}
...
func (hs *HandlerService) GetRatedContents(c *gin.Context) {
	...
	var pagination PaginationResult
	var totalCount int
	if err := db.Table("content c").Select("c.content_key::text").Joins("join rated_content rc on rc.content_id =c.id").Where("rc.user_id=? and rc.is_hidden=false", UserId).Order("rc.rated_at desc").Count(&totalCount).Error; err != nil {
		c.JSON(http.StatusInternalServerError, serverError)
		return
	}
	if err := db.Table("content c").Select("c.content_key::text").Joins("join rated_content rc on rc.content_id =c.id").Where("rc.user_id=? and rc.is_hidden=false", UserId).Order("rc.rated_at desc").Limit(limit).Find(&contentKeys).Error; err != nil {
		c.JSON(http.StatusInternalServerError, serverError)
		return
	}
	for _, cKey := range contentKeys {
...
func (hs *HandlerService) GetWatchingContents(c *gin.Context) {
	...
	var pagination PaginationResult
	var totalCount int
	if err := db.Table("content c").Select("c.content_key::text").Joins("join view_activity va on va.content_id =c.id and va.user_id=?", UserId).Where("va.user_id=? and va.is_hidden=?", UserId, false).Order("va.viewed_at desc").Count(&totalCount).Error; err != nil {
		c.JSON(http.StatusInternalServerError, serverError)
		return
	}
	if err := db.Table("content c").Select("c.content_key::text").Joins("join view_activity va on va.content_id =c.id and va.user_id=?", UserId).Where("va.user_id=? and va.is_hidden=?", UserId, false).Order("va.viewed_at desc").Limit(limit).Find(&contentKeys).Error; err != nil {
		c.JSON(http.StatusInternalServerError, serverError)
		return
	}
	for _, cKey := range contentKeys {
	...
	if err := db.Raw(query).Find(&result).Error; err == nil {
		...
		if result.ContentTier == 1 {
			...
			if episode == 1 && UserId != "" {
				...
				if err := db.Table("view_activity va").Select(fields).Joins(join).Where(where, result.Id, UserId).Order("va.viewed_at desc").Limit(1).Find(&viewActivity).Error; err == nil {
		...
		if err := db.Table("view_activity").Select("viewed_at,last_watch_position").Where("content_id =? and user_id =? and is_hidden=false", result.Id, UserId).Order("viewed_at desc").Limit(1).Find(&activityDetails).Error; err == nil {
		...
		if UserId != "" {
			...
			var playlistedContent PlaylistedContent
			if err := db.Where("user_id=? and content_id=?", UserId, result.Id).Find(&playlistedContent).Error; err == nil {
				userRating.AddedToPlaylistAt = &playlistedContent.AddedAt
			}
			var ratedContent RatedContent
			if err := db.Where("user_id=? and content_id=? and is_hidden=false", UserId, result.Id).Find(&ratedContent).Error; err == nil {
...
func (hs *HandlerService) AddViewActivity(c *gin.Context) {
	...
	if request.Content.ContentType != "LiveTV" {
		...
		if rows := db.Table("view_activity").Where("playback_item_id=? and content_id=? and user_id=?", cDetails.PlaybackItemId, cDetails.Id, UserId).Find(&viewActivity).RowsAffected; rows != 0 {
			update.IsHidden = "false"
			update.ViewedAt = time.Now()
			update.LastWatchPosition = request.LastWatchPosition
			update.WatchSessionId = request.WatchSessionId
			update.DeviceId = deviceId.(string)
			if err := db.Model(&update).Where("id=?", viewActivity.Id).Update(&update).Error; err != nil {
				c.JSON(http.StatusInternalServerError, serverError)
				return
			}
			if err := db.Model(&update).Where("id=?", viewActivity.Id).Update(map[string]interface{}{"is_hidden": false, "device_id": deviceId.(string)}).Error; err != nil {
				c.JSON(http.StatusInternalServerError, serverError)
				return
			}
		...
		// userEvent.EventType = "watching_history_activity"
		...
		// 	return
		// }
		// fmt.Println("event response - ", string(response))
	}
...
func (hs *HandlerService) GetsearchContent(c *gin.Context) {
	...
	// 	where
	// 		c.deleted_by_user_id is null
	// 		and c.status = 1
	// 		and (lower(cpi.transliterated_title) like '%` + querystring + `%'
	// 			or lower(cpi.arabic_title) like '%` + querystring + `%')
	// `
...
func (hs *HandlerService) AddUserPlaylist(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&request); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "invalid_request", "description": "Invalid request", "code": "invalid_request", "requestId": userid.(string)})
		return
	}
	var content Content
...
func (hs *HandlerService) AddRatingForContentByUser(c *gin.Context) {
	...
	if idresults := db.Table("content c").Select("c.id as id").Where("content_key=(?)", addcontent.ContentRequest.Id).Find(&contentid).Error; idresults != nil {
		c.JSON(http.StatusBadRequest, badRequestError)
		return
	}
	/*collect rated_content details based on user_id & content_id*/
	var count int
	if idresults := db.Table("rated_content").Select("rating").Where("content_id=? and user_id=?", contentid.Id, userId).Count(&count).Error; idresults != nil {
		c.JSON(http.StatusInternalServerError, serverError)
		return
	}
	...
	if newHistory := db.Table("rated_content_history").Create(&addToHistory).Error; newHistory != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"message": newHistory.Error(), "status": http.StatusInternalServerError})
		return
	}
	/* Find most-recent average_rating based on content_id */
	var average float64
	row := db.Raw("select avg(rating) from rated_content where content_id =?", contentid.Id)
	row.Count(&average)
...
func (hs *HandlerService) RemoveContentsUserPlaylist(c *gin.Context) {
	...
	fmt.Println(userId, "............")
	if len(res) != 2 {
		c.JSON(http.StatusBadRequest, gin.H{"message": "please provide contentkey and contenttype"})
	}
	...
	if result := cdb.Table("content c").Select("id").
		...
		c.JSON(http.StatusNotFound, gin.H{"error": "not_found", "description": "Not found", "code": "", "requestId": randstr.String(32)})
		return
	}
...
func (hs *HandlerService) RemoveWatchingHistory(c *gin.Context) {
	...
	update.IsHidden = "true"
	update.ViewedAt = time.Now()
	if err := db.Model(&update).Where("playback_item_id=? and content_id=? and user_id=?", cDetails.PlaybackItemId, cDetails.Id, UserId).Update(&update).Error; err != nil {
		c.JSON(http.StatusInternalServerError, serverError)
		return
	}
}
...
// @Success 200 {object} object	PlaylistDetailsResponse
// @Failure 400 {object} object err
// @Failure 404 {object} object err
// @Failure 500 {object} object err
...
func (hs *HandlerService) GetPlaylistDetails(c *gin.Context) {
	...
	if playlist.PlaylistType == "pagecontent" {
		...
		contentIds, err := menu.PlaylistItemContents(playlist.ID, c)
		if err != nil {
			c.JSON(http.StatusInternalServerError, serverError)
			return
		}
