...
func (hs *HandlerService) TempHomePage(c *gin.Context) {
	...
	c.JSON(http.StatusOK, s3response)
}
...
func (hs *HandlerService) TvHomePage(c *gin.Context) {
	...
	if RedisResponse.Value != "" {
		...
		url := os.Getenv("BASE_URL") + language + "/menu?device=" + device + "&country=" + country
		method := "GET"
		client := &http.Client{}
		...
		for _, singlePage := range menuPage.Data {
			...
			url := os.Getenv("BASE_URL") + language + "/menu/" + strconv.Itoa(singlePage.ID) + "?cascade=2&country=" + country
			method := "GET"
			fmt.Println(url)
			client := &http.Client{}
			req, err := http.NewRequest(method, url, nil)
