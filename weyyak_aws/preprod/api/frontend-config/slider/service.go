...
func (hs *HandlerService) GetSliderList(c *gin.Context) {
	...
	// 		c.JSON(http.StatusOK, gin.H{"pagination": finalredisResponse.Pagination, "data": finalredisResponse.Finalsliderlist})
	// 		return
	// 	}
	// }
	...
	// order by removed for english_name
	rawquery := "select s.id,s.name,s.is_disabled,s.scheduling_end_date,string_agg(distinct p.english_title::text, ',')::varchar as found_in,string_agg(distinct case when ps.order = 0 then 'true' else 'false' end,',')::boolean as is_default_for_any_page,case when count(distinct pp.id) = 10 then 'All Platforms' else string_agg(distinct pp.platform :: varchar, ',') end as available_on,case when count(distinct c.english_name)= 248 then 'All Regions' else string_agg(distinct c.english_name ::varchar, ',' ) end as region,case when count(distinct sc.country_id)<5 or count(distinct sc.country_id)= 248 then 'false' else 'true' end as has_more_regions from slider s left join page_slider ps on ps.slider_id =s.id left join page p on p.id =ps.page_id join slider_country sc on s.id = sc.slider_id join country c on sc.country_id = c.id and sc.slider_id = s.id join slider_target_platform stp on s.id = stp.slider_id join publish_platform pp on stp.target_platform = pp.id where s.deleted_by_user_id is null and s.name != ''"
