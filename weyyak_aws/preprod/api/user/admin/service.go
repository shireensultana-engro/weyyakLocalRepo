...
func (hs *HandlerService) CreateAdminDetails(c *gin.Context) {
	...
	/*End for Validations*/
	user := register.User{Email: request.Email, FirstName: request.FirstName, LastName: request.LastName, IsBackOfficeUser: true, LanguageId: 1, IsAdult: true, LastActivityAt: time.Now(), RegisteredAt: time.Now(), EmailConfirmed: true, Version: 2, UserName: request.Email, RoleId: "90f15b92-97fd-e611-814f-0af7afba4acb"}
	...
	token := "thisNotCompleted"
	templateData := struct {
		PasswordChangeUrl string
	}{
		PasswordChangeUrl: os.Getenv("PASSWORDCHANGEURL") + "email=" + request.Email + "&token=" + token,
	}
...
func (hs *HandlerService) GetAllAdminDetails(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"pagination": pagination, "data": userKeys})
	return
}
...
func (hs *HandlerService) DeleteAdminDetails(c *gin.Context) {
	...
	if UserId == c.Param("id") {
		c.JSON(http.StatusBadRequest, gin.H{"Message": " Not allowed to delete himself."})
		return
	}
	db := c.MustGet("DB").(*gorm.DB)
	var totalCount int
...
func (hs *HandlerService) UpdateAdminDetails(c *gin.Context) {
	...
	db.Table("public.user").Where("id = ?", c.Param("id")).Updates(map[string]interface{}{"email": request.Email, "first_name": request.FirstName, "last_name": request.LastName, "password_hash": passwordHash, "salt_stored": saltStored, "version": 2})
	return
}
...
func (hs *HandlerService) ResetAdminPassword(c *gin.Context) {
	...
	if count < 1 {
		...
		c.JSON(http.StatusBadRequest, emailError)
		return
	}
	/*End for Validations*/
	...
	templateData := struct {
		...
		PasswordChangeUrl: "https://wyk2-me-preprodbo1.weyyak.com/reset-password?" + "email=" + request.Email + "&resetPasswordToken=" + token,
	}
	templatePath := "BackOfficeUserSetupPasswordBody.html"
	con, _ := ioutil.ReadFile(templatePath)
	content := string(con)
