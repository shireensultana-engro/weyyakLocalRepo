...
func (hs *HandlerService) RegisterUserUsingSMS(c *gin.Context) {
	...
	if phonenumbercheck.PhoneNumberConfirmed {
		...
		c.JSON(http.StatusBadRequest, phoneError)
		return
	}
...
// @Produce  json
// @security Authorization
// @Param body body UsercodeRequest true "Raw JSON string"
// @Success 200 {array} object c.JSON
...
func (hs *HandlerService) SendOtp(c *gin.Context) {
	...
	if forcount.Number < 6 {
		...
		if lastSent < currentTime {
			...
			if result := db.Table("otp_record").Where("phone=(?)", forcount.Phone).Update(UserDetails{Message: otp, SentOn: time.Now(), Number: 1}).Error; result != nil {
				c.JSON(http.StatusInternalServerError, gin.H{"message": result.Error(), "status": http.StatusInternalServerError})
				return
			}
		} else {
...
func (hs *HandlerService) ForgotPasswordOtp(c *gin.Context) {
	...
	if userphoneresult := db.Table("user").Where("national_number=(?)", verify.PhoneNumber).Find(&user).Error; userphoneresult != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"message": userphoneresult.Error(), "status": http.StatusInternalServerError})
		return
	}
...
func (hs *HandlerService) UsersListandSearchbyFilterswithPagination(c *gin.Context) {
	...
	if err == nil {
		...
		c.JSON(http.StatusOK, gin.H{"pagination": redisResponse.Pages, "data": redisResponse.Data})
		return
	}
...
func (hs *HandlerService) DeleteUserDevices(c *gin.Context) {
	...
	//var userdevice UserDevice
	db.Exec("DELETE FROM oauth2_tokens ot  WHERE ot.data->>'DeviceID'=? and ot.data->> 'UserID' = ?;", c.Param("deviceid"), c.MustGet("userid"))
