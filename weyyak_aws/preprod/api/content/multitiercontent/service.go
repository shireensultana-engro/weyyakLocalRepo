...
func (hs *HandlerService) CreateOrUpdatePublishedOnetierContentDetails(c *gin.Context) {
	...
	for _, data := range *request.TextualData.ContentVariances {
		...
		if len(data.PublishingPlatforms) == 0 {
			c.JSON(http.StatusBadRequest, common.ServerError{Error: "publishing platforms empty", Description: "Publishing platform is empty, Atleast one platform need to be assigned before publish", Code: "", RequestId: randstr.String(32)})
			return
		}
	}
	var errorFlags bool
				...
				for i, variancerange := range value.VarianceTrailers {
					...
					if variancerange.Id != "" {
						...
						if variancerange.VideoTrailerId != "" {
							...
							go ContentVarianceTrailerImageUpload(c.Param("id"), varianceTrailerCreate.ContentVarianceId, varianceTrailerCreate.Id, variancerange.TrailerPosterImage)
						}
					}
				}
