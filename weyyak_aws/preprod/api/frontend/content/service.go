...
// @Param contentType platform string true "Platform"
// @Success 200 {array} object c.JSON
...
func stringInSlice(a string, list []string) bool {
	for _, b := range list {
		log.Println("printing the values for slice in sting function--", list, a)
		if b == a {
			return true
		}
	}
...
func (hs *HandlerService) GetPlaylistDetails(c *gin.Context) {
	...
	if playlist.PlaylistType == "pagecontent" {
		...
		var content1 Cont
		if contentError := fdb.Debug().Raw("select jsonb_agg(details) as cnt from content_fragment cf where cf.content_id in (?) and (?) = ANY(string_to_array(replace(replace(cf.country, '[', ''), ']', ''), ', ')) and language in (?)  and (select cid from (select distinct unnest(translate::text[]) as cid from (select (translate(country,'[]','{}')) from content_fragment cf where cf.content_id in (?)) as ss )aa where cid= ? )= ? and (select cid from (select distinct unnest(translate::text[]) as cid from (select (translate(platform,'[]','{}')) from content_fragment cf where cf.content_id in (?)) as ss )aa where cid= ? )= ? and rights_start_date <= now() and rights_end_date >= now()", contentIdss, countryCodeInt, language, contentIdss, "356", "356", contentIdss, "1", "1").Find(&content1).Error; contentError != nil {
