...
func (hs *ContentService) Bootstrap(r *gin.Engine) {
	...
	qrg.POST("/nontextual/content/poster-image", hs.UploadMenuPosterImage)
	qrg.POST("/nontextual/content/details-background", hs.DetailsBackgroundImage)
	qrg.POST("/nontextual/content/mobile-details-background", hs.MobileDetailsBackgroundImage)
	qrg.POST("/nontextual/content/overlay-poster-image", hs.OverlayPosterImage)
	qrg.POST("/nontextual/content/menu-poster-image", hs.MenuPosterImage)
	qrg.POST("/nontextual/content/mobile-menu", hs.MobileMenu)
	qrg.POST("/nontextual/content/mobile-menu-poster-image", hs.MobileMenuPosterImage)
	qrg.POST("/nontextual/content/trailer-poster-image", hs.TrailerPosterImage)
}
...
func (hs *ContentService) CreateSeason(c *gin.Context) {
	...
	if c.Param("id") == "" && request.SeasonId == "" {
		...
		if len(*request.VarianceTrailers) != 0 {
			for i, trailerrange := range *request.VarianceTrailers {
				if trailerrange.VideoTrailerId != "" {
					...
					go SeasonTrailerImageUpload(*request.ContentId, season.Id, varianceTrailer.Id, trailerrange.TrailerPosterImage)
				}
			}
		}
		...
		seasonId := season.Id
		/* Image Upload */
		go SeasonFileUPload(request, seasonId, season.ContentId)
		/* Fragment Creation */
		...
		if len(*request.VarianceTrailers) != 0 {
			for i, trailerrange := range *request.VarianceTrailers {
				if trailerrange.VideoTrailerId != "" {
					...
					go SeasonVarianceTrailerImageUpload(*request.ContentId, request.SeasonId, trailerrange.Id, *request.ContentId, season.Id, varianceTrailer.Id, trailerrange.TrailerPosterImage)
				}
			}
		}
		...
		for _, data := range episode {
			...
			go SeasonVarianceEpisodeImageUpload(*request.ContentId, season.Id, episodeDetails.Id, image, *request.ContentId, request.SeasonId, data.Id)
		}
		/*commit changes*/
		if err := tx.Commit().Error; err != nil {
			l.JSON(c, http.StatusInternalServerError, serverError)
		...
		seasonId := season.Id
		SeasonVarianceFileUPload(request.SeasonId, seasonId, season.ContentId)
		/* Fragment Creation */
		go fragments.CreateContentFragment(season.ContentId, c)
		/* Prepare Redis Cache for all contents */
		db.Debug().Raw("select content_key,content_type from content where id=?", season.ContentId).Find(&seasondetails)
...
func (hs *ContentService) DraftSeason(c *gin.Context) {
	...
	if c.Param("id") == "" && request.SeasonId == "" {
		...
		seasonId := season.Id
		SeasonFileUPload(request, seasonId, season.ContentId)
		/* Prepare Redis Cache for all contents */
		db.Debug().Raw("select content_key,content_type from content where id=?", season.ContentId).Find(&seasondetails)
		/* Prepare Redis Cache for single content*/
		contentkeyconverted := strconv.Itoa(seasondetails.ContentKey)
		...
		for i, trailerrange := range *request.VarianceTrailers {
			if trailerrange.Id == "" {
				if trailerrange.VideoTrailerId != "" {
					...
					orders = orders + 1
					go SeasonTrailerImageUpload(*request.ContentId, c.Param("id"), varianceTrailer.Id, trailerrange.TrailerPosterImage)
				}
			} else {
				...
				if trailerrange.VideoTrailerId == "" {
					...
					go SeasonTrailerImageUpload(*request.ContentId, c.Param("id"), trailerrange.Id, trailerrange.TrailerPosterImage)
				}
			}
		}
	}
